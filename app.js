...
const investmentAccountTypes=['IRA','401k','Brokerage'];
...
document.querySelectorAll('.tab').forEach(b=>b.onclick=()=>{document.querySelectorAll('.tab').forEach(x=>x.classList.remove('active'));document.querySelectorAll('.app > section').forEach(x=>x.classList.add('hidden'));b.classList.add('active');document.getElementById(b.dataset.tab).classList.remove('hidden')});monthPicker.onchange=e=>{active=e.target.value;render()};document.addEventListener('change',e=>{const el=e.target;if(el.dataset.month){const month=data.months.find(x=>x.id===el.dataset.month);month[el.dataset.key]=num(el.value);save();syncMonth(month);render()}if(el.dataset.setting){data.settings[el.dataset.setting]=num(el.value);save();syncSettings();render()}});
...
let latestLocalSettingsUpdate=0;const settingsTimestamp=value=>{const numeric=Number(value);return Number.isFinite(numeric)?numeric:(Date.parse(value)||0)};async function syncSettings(){if(!firebaseClient||!firebaseUser)return;latestLocalSettingsUpdate=Date.now();try{await firebaseClient.setDoc(firebaseClient.doc(firebaseClient.db,...firebasePath('budget'),'settings'),{settings:{...data.settings},updatedAt:latestLocalSettingsUpdate})}catch(error){console.error(error);firebaseStatusMessage('Could not sync Settings. Your local changes are still saved here.')}}
...
function subscribeFirebase(){
  ...
  firebaseUnsubscribers.push(firebaseClient.onSnapshot(investmentsRef,snapshot=>{const migrated=[];data.investments=snapshot.docs.map(record=>{const raw=record.data();if(raw?.currentTotal!==undefined&&raw?.currentTotal!==null&&raw?.currentTotal!==''){const balance=normalizeInvestmentBalanceRecord({id:`${raw.accountId}-${raw.date}`,accountId:raw.accountId,date:raw.date,total:raw.currentTotal,createdAt:raw.createdAt,updatedAt:raw.updatedAt});if(balance)migrated.push(balance)}return normalizeInvestmentRecord(raw,record.id)}).filter(Boolean).sort((a,b)=>String(b.date).localeCompare(String(a.date))||String(b.createdAt).localeCompare(String(a.createdAt)));migrated.forEach(balance=>{const index=data.investmentBalances.findIndex(item=>item.accountId===balance.accountId&&item.date===balance.date);if(index>=0)data.investmentBalances[index]=balance;else data.investmentBalances.push(balance);syncInvestmentBalanceRecord(balance)});save();render()}));
...
function monthTable(months){return `<div class="table-wrap"><table class="table monthly-table"><thead><tr><th>Month</th><th>Saved</th><th>Spending</th><th>Food</th><th>Water</th><th>Electricity</th><th>Gas</th><th>Megan owes</th><th aria-label="Actions"></th></tr></thead><tbody>${months.map(month=>{const values=total(month),label=`${monthLabels[Number(month.id.slice(5,7))-1].slice(0,3)} '${month.id.slice(2,4)}`;return `<tr><th>${label}</th><td class="saved ${savedTone(values.saved)}">${money(values.saved)}</td><td><div class="spending-cell"><input type="number" data-month="${month.id}" data-key="spending" value="${round(month.spending)}"><button class="notes-button ${month.notes?'has-notes':''}" data-notes-month="${month.id}" aria-label="Edit notes for ${month.label}" title="${month.notes?'Edit notes':'Add notes'}">▤</button></div></td><td><input type="number" data-month="${month.id}" data-key="groceries" value="${round(month.groceries)}"></td>${['water','electricity','gas'].map(key=>`<td class="${utilityValueMissing(month,key)?'utility-missing':''}"><input type="number" data-month="${month.id}" data-key="${key}" value="${round(month[key])}"></td>`).join('')}<td class="megan">${money(values.megan)}</td><td class="actions"><details><summary aria-label="More actions">•••</summary><div class="action-menu"><button data-offset-month="${month.id}">Add cash offset</button></div></details></td></tr>`}).join('')}</tbody></table></div>`}
...
const backupReminderRender=render;render=()=>{backupReminderRender();setupBackupTools();maybeShowMeganReminder()};render();function utilityValueMissing(month,key){const now=new Date(),offset=key==='water'?-2:-1,cutoff=`${new Date(now.getFullYear(),now.getMonth()+offset,1).getFullYear()}-${String(new Date(now.getFullYear(),now.getMonth()+offset,1).padStart?new Date(now.getFullYear(),now.getMonth()+offset,1).getMonth()+1:new Date(now.getFullYear(),now.getMonth()+offset,1).getMonth()+1).padStart(2,'0')}`;return month.id<=cutoff&&num(month[key])===0}function renderUtilityAlert(){const alert=document.getElementById('utilityAlert');if(!alert)return;const names={water:'Water',electricity:'Electricity',gas:'Gas'},missing=['water','electricity','gas'].map(key=>[key,data.months.filter(month=>utilityValueMissing(month,key))]).filter(([,months])=>months.length);alert.classList.toggle('hidden',!missing.length);if(!missing.length){alert.textContent='';return}alert.textContent=`Utility values still needed — ${missing.map(([key,months])=>`${names[key]}: ${months.map(month=>`${monthLabels[Number(month.id.slice(5,7))-1].slice(0,3)} '${month.id.slice(2,4)}`).join(', ')}`).join(' · ')}`};const utilityAlertRender=render;render=()=>{utilityAlertRender();renderUtilityAlert()};async function addCloudExpense(entry,keyName,notes){if(!firebaseClient||!firebaseUser)return;const monthRef=firebaseClient.doc(firebaseClient.db,...firebasePath('months'),entry.month),entryRef=firebaseClient.doc(firebaseClient.db,...firebasePath('expenses'),entry.id);try{await firebaseClient.runTransaction(firebaseClient.db,async transaction=>{const snapshot=await transaction.get(monthRef),remote=cleanMonth(snapshot.exists()?snapshot.data():freshMonth(entry.month));remote[keyName]=round(remote[keyName]+entry.final);if(notes!==undefined)remote.notes=notes;transaction.set(monthRef,{...remote,updatedAt:new Date().toISOString()})});await firebaseClient.setDoc(entryRef,entry)}catch(error){console.error(error);firebaseStatusMessage('Could not sync this expense yet. It remains saved on this device.')}}saveExpense.onclick=()=>{const original=num(expenseAmount.value),modifier=Math.max(.9,Math.min(1,num(expenseModifier.value)));if(original<=0)return notice.textContent='Enter a positive expense amount.';const final=round(original*modifier),type=expenseType.value,keyName=type.toLowerCase(),comment=type==='Spending'?expenseComment.value.trim():'',entry={id:crypto.randomUUID(),type,month:expenseMonth.value,original,modifier,final,comment,createdAt:new Date().toISOString()},month=data.months.find(item=>item.id===entry.month);month[keyName]=round(month[keyName]+final);const notesChanged=original>200&&comment?appendExpenseCommentToNotes(month,entry):false;data.entries.unshift(entry);addCloudExpense(entry,keyName,notesChanged?month.notes:undefined);active=entry.month;save();dialog.close();expenseAmount.value='';expenseComment.value='';notice.textContent=money(final)+' added to '+type.toLowerCase()+'.';render()};render();
  ...
  (() => {
    ...
    typeLabel.innerHTML = '<span>Type</span><div class="option-grid expense-category-options" id="expenseCategoryOptions"><button type="button" class="choice-option" data-category="Groceries">Groceries</button><button type="button" class="choice-option" data-category="Restaurants">Restaurants</button><button type="button" class="choice-option selected" data-category="Shopping">Shopping</button></div><div class="expense-suboptions" id="shoppingSuboptions"><span>Shopping type</span><div class="option-grid shopping-suboptions"><button type="button" class="choice-option selected" data-shopping="Dining">Dining</button><button type="button" class="choice-option" data-shopping="Entertainment">Entertainment</button><button type="button" class="choice-option" data-shopping="Travel">Travel</button><button type="button" class="choice-option" data-shopping="Health">Health</button><button type="button" class="choice-option" data-shopping="Pets">Pets</button><button type="button" class="choice-option" data-shopping="Misc">Misc</button></div></div>';
    ...
    saveExpense.onclick = () => {
      ...
      const entry = { id: crypto.randomUUID(), type, category, subcategory: category === 'Groceries' || category === 'Restaurants' ? '' : subcategory, month: month.value, original, modifier: appliedModifier, final: finalAmount, comment: entryComment, createdAt: new Date().toISOString() };
  ...
  (() => {
    ...
    const setCategory = (nextCategory, nextSubcategory) => {
      ...
      subcategory = nextSubcategory || (category === 'Bills' ? 'Water' : 'Dining');
    ...
    const loadEntry = entry => {
      ...
      const inferredSubcategory = entry.subcategory || (inferredCategory === 'Bills' ? entry.type : 'Dining');
    ...
    form.addEventListener('submit', event => {
      ...
      Object.assign(entry, { original, modifier: appliedModifier, type: newType, category, subcategory: category === 'Groceries' || category === 'Restaurants' ? '' : subcategory, month: month.value, final: newFinal, comment: newType === 'Spending' ? comment.value.trim().slice(0, 500) : '', updatedAt: new Date().toISOString() });
      save();
  ...
  (() => {
    ...
    const setupExpenseSettingsTabs = () => {
      ...
      if (!host) {
        const fields = ['fixedExpenses','kevinMortgage','foodShare','meganR1','meganR2'].map(key => document.querySelector(`[data-setting="${key}"]`)?.closest('label')).filter(Boolean);
        host = document.createElement('section');
        host.id = 'expenseSettingsTabs';
        host.className = 'expense-settings-tabs';
        host.innerHTML = '<div class="settings-tab-list" role="tablist"><button type="button" class="settings-tab active" data-expense-settings-tab="one" role="tab" aria-selected="true">Expenses 1</button><button type="button" class="settings-tab" data-expense-settings-tab="two" role="tab" aria-selected="false">Expenses 2</button></div><div class="settings-tab-panel" data-expense-settings-panel="one"></div><div class="settings-tab-panel" data-expense-settings-panel="two" hidden></div>';
        ...
        const helper = document.createElement('p'); helper.id = 'fixedExpensesHelp'; helper.className = 'small'; helper.textContent = 'Calculated from Expenses 2.'; fixedInput.closest('label').append(helper);
  ...
  (() => {
    ...
    const accountYearContribution=(account,year)=>is401k(account)?round(num(account.yearlyContribution)/12*inferred401kMonths(year)):round(contributions().filter(entry=>entry.accountId===account.id&&entry.date.startsWith(year)).reduce((sum,entry)=>sum+entry.amount,0));
    ...
    const balancePoints=account=>balances(account.id).map(entry=>({label:new Date(`${entry.date}T12:00:00`).toLocaleDateString('en-US',{month:'short',year:'2-digit'}),value:num(entry.total)}));
    ...
    const investmentLineChart=(points,color,label,emptyText)=>{
      ...
      const width=520,height=180,left=48,right=12,top=12,bottom=28,plotWidth=width-left-right,plotHeight=height-top-bottom,max=Math.max(1,...points.map(point=>point.value)),x=index=>points.length===1?left+plotWidth/2:left+index*plotWidth/(points.length-1),y=value=>top+(max-value)*plotHeight/max,path=points.map((point,index)=>`${index?'L':'M'} ${x(index).toFixed(1)} ${y(point.value).toFixed(1)}`).join(' '),indexes=[...new Set([0,Math.floor((points.length-1)/2),points.length-1])],gradientId=`investment-fill-${chartSequence++}`;
    ...
    const pieStyle=values=>{const total=values.reduce((sum,value)=>sum+value,0);if(!total)return'var(--soft)';let start=0;return`conic-gradient(${values.map((value,index)=>{const from=start;start+=value/total*100;return`${colorAt(index)} ${from}% ${start}%`}).join(',')})`};
    const pieCard=(title,subtitle,accounts,values)=>{const total=round(values.reduce((sum,value)=>sum+value,0));return `<article class="investment-summary-card"><div><h3>${title}</h3><p>${subtitle}</p><div class="investment-legend">${accounts.map((account,index)=>`<span><i style="--legend-color:${colorAt(index)}"></i>${escapeHtml(account.name)} · ${money(values[index])}</span>`).join('')}</div></div><div class="investment-donut" style="--investment-pie:${pieStyle(values)}" role="img" aria-label="${escapeHtml(title)}: ${money(total)}"><strong class="investment-donut-total">${money(total)}</strong></div></article>`};
    ...
    const eligibleContributionAccounts=()=>activeAccounts().filter(account=>!is401k(account));
    ...
    const settingsRow=account=>{const goalField=is401k(account)?'yearlyContribution':'annualGoal',goalLabel=is401k(account)?'Yearly Contribution':'Yearly Goal',goalValue=round(account[goalField]);return `<div class="investment-settings-row${account.archived?' is-archived':''}"><label>Account name<input type="text" maxlength="60" value="${escapeHtml(account.name)}" data-investment-setting="name" data-investment-account-id="${account.id}"></label><label>Type<select data-investment-setting="type" data-investment-account-id="${account.id}">${investmentAccountTypes.map(type=>`<option${account.type===type?' selected':''}>${type}</option>`).join('')}</select></label><label>${goalLabel}<input type="number" min="0" step="0.01" inputmode="decimal" value="${goalValue}" data-investment-setting="${goalField}" data-investment-account-id="${account.id}"></label><div class="investment-settings-actions"><button type="button" class="button ghost" data-archive-investment-account="${account.id}">${account.archived?'Restore':'Archive'}</button></div></div>`};
    const renderInvestments=()=>{
      if(!section)return;chartSequence=0;const today=localToday(),year=today.slice(0,4),month=today.slice(0,7),accounts=activeAccounts(),yearValues=accounts.map(account=>accountYearContribution(account,year)),balanceValues=accounts.map(currentAccountTotal);
      summary.innerHTML=pieCard('Total Portfolio Value','Latest balance update',accounts,balanceValues)+pieCard('Total Added This Year',`Cash contributions through ${new Date(`${month}-01T12:00:00`).toLocaleDateString('en-US',{month:'long'})}`,accounts,yearValues);
      accountGrid.innerHTML=accounts.length?accounts.map((account,index)=>{const yearContribution=yearValues[index],currentTotal=balanceValues[index],goal=is401k(account)?num(account.yearlyContribution):num(account.annualGoal),progress=!is401k(account)&&goal?Math.min(100,yearContribution/goal*100):0,color=colorAt(index);return `<article class="investment-account-card"><div class="investment-account-info"><div class="investment-account-heading"><h3>${escapeHtml(account.name)}</h3><span class="investment-account-type">${escapeHtml(account.type)}</span></div><div class="investment-account-stats"><span class="investment-account-stat balance"><span>Current balance</span><strong>${money(currentTotal)}</strong></span><span class="investment-account-stat"><span>Added this year</span><strong>${money(yearContribution)}</strong></span><span class="investment-account-stat"><span>${is401k(account)?'Yearly contribution':'Yearly goal'}</span><strong>${goal?money(goal):'Not set'}</strong></span></div></div>${!is401k(account)&&goal?`<div class="investment-progress" aria-label="${Math.round(progress)}% of yearly goal"><i style="width:${progress}%;background:${color}"></i></div><p class="investment-goal-label"><span>${Math.round(progress)}%</span><span>${money(goal)}</span></p>`:''}<div class="investment-account-charts"><section class="investment-chart"><h4>Contributions · ${year}</h4><p>${is401k(account)?'Monthly amount inferred from Yearly Contribution':'Monthly cash contributions'}</p>${investmentLineChart(contributionPoints(account,year),color,`${account.name} monthly contributions for ${year}`,'Contributions will appear here.')}</section><section class="investment-chart"><h4>Total amount</h4><p>Point-in-time balance updates</p>${investmentLineChart(balancePoints(account),color,`${account.name} total balance over time`,'Use Update totals to begin this graph.')}</section></div></article>`}).join(''):'<div class="chart-empty">Add an investment account in Investment Settings.</div>';
      ...
      const active=data.settings.investmentAccounts.filter(account=>!account.archived),archived=data.settings.investmentAccounts.filter(account=>account.archived);settingsHost.innerHTML=active.map(settingsRow).join('')+(archived.length?`<h4 class="investment-archived-heading">Archived</h4>${archived.map(settingsRow).join('')}`:'');
    };
    ...
    accountForm.addEventListener('submit',event=>{event.preventDefault();const account=normalizeInvestmentAccount({id:`investment-account-${crypto.randomUUID()}`,name:newAccountName.value,type:newAccountType.value,annualGoal:0,yearlyContribution:0,currentTotal:0,archived:false,createdAt:new Date().toISOString()});if(!account)return;data.settings.investmentAccounts.push(account);save();syncSettings();accountDialog.close();render()});
    settingsHost.addEventListener('change',event=>{const input=event.target.closest('[data-investment-setting]');if(!input)return;const account=accountById(input.dataset.investmentAccountId);if(!account)return;const field=input.dataset.investmentSetting;if(field==='name')account.name=safeText(input.value||'Account',60).trim()||'Account';else if(field==='type'&&investmentAccountTypes.includes(input.value)){account.type=input.value;if(is401k(account))account.annualGoal=0;else account.yearlyContribution=0}else account[field]=safeNumber(input.value,0);save();syncSettings();render()});
    settingsHost.addEventListener('click',event=>{const button=event.target.closest('[data-archive-investment-account]');if(!button)return;const account=accountById(button.dataset.archiveInvestmentAccount);if(!account)return;if(!account.archived&&!confirm(`Archive ${account.name}? Its contribution and balance history will be preserved.`))return;account.archived=!account.archived;save();syncSettings();render()});
