const localKeys={data:'estuary-v1',displayMode:'estuary-display-mode',firebaseConfig:'estuary-firebase-config',pkce:'estuary-pkce',theme:'estuary-theme'},legacyPrefix=['cream','corn'].join('-');Object.entries(localKeys).forEach(([name,newKey])=>{const oldKey=name==='data'?`${legacyPrefix}-v1`:`${legacyPrefix}-${name==='firebaseConfig'?'firebase-config':name==='displayMode'?'display-mode':name}`;const oldValue=localStorage.getItem(oldKey);if(localStorage.getItem(newKey)===null&&oldValue!==null)localStorage.setItem(newKey,oldValue);if(oldValue!==null)localStorage.removeItem(oldKey)});const key=localKeys.data,monthLabels=['January','February','March','April','May','June','July','August','September','October','November','December'], round=n=>Math.round((Number(n)||0)*100)/100, maskDigits=value=>String(value).replace(/\d/g,'9'), num=x=>Number.isFinite(Number(x))?round(x):0;let displayMode=localStorage.getItem(localKeys.displayMode)==='on',historyFilter='',money=n=>{const value=new Intl.NumberFormat('en-US',{style:'currency',currency:'USD',maximumFractionDigits:2}).format(round(n));return displayMode?maskDigits(value):value};
...
function normalizeMonthRecord(value,fallbackId=''){if(!value||typeof value!=='object'||Array.isArray(value))return null;const id=safeMonthId(value.id)||safeMonthId(fallbackId);if(!id)return null;const monthIndex=Number(id.slice(5,7))-1,record={id,label:`${monthLabels[monthIndex]} ${id.slice(0,4)}`,water:safeNumber(value.water),electricity:safeNumber(value.electricity),gas:safeNumber(value.gas),groceries:safeNumber(value.groceries),spending:safeNumber(value.spending),cashOffset:safeNumber(value.cashOffset),notes:safeText(value.notes,500)};if(value.savedLocked!==undefined&&value.savedLocked!==null&&value.savedLocked!=='')record.savedLocked=safeNumber(value.savedLocked);if(value.savedLockedAt)record.savedLockedAt=safeDateTime(value.savedLockedAt);if(Number.isFinite(Number(value.incomePaycheck)))record.incomePaycheck=safeNumber(value.incomePaycheck,0);return record}
...
let firebaseClient=null,firebaseUser=null,firebaseUnsubscribers=[],clearPrivateDataAfterAuthChange=false,firebaseSettingsReady=false,firebaseMonthsReady=false;
...
function fourMonthSavedCutoff(){const now=new Date(),cutoff=new Date(now.getFullYear(),now.getMonth()-4,1);return`${cutoff.getFullYear()}-${String(cutoff.getMonth()+1).padStart(2,'0')}`}
function maybeLockSavedMonths(){if(!firebaseUser||!firebaseSettingsReady||!firebaseMonthsReady)return false;const cutoff=fourMonthSavedCutoff(),lockedAt=new Date().toISOString(),changed=[];data.months.forEach(month=>{if(month.id<=cutoff&&(month.savedLocked===undefined||month.savedLocked===null||month.savedLocked==='')){month.savedLocked=calculatedMonthValues(month).saved;month.savedLockedAt=lockedAt;changed.push(month)}});if(!changed.length)return false;save();changed.forEach(syncMonth);return true}
...
const refreshSavedLocks=()=>{if(maybeLockSavedMonths())render()};
document.addEventListener('visibilitychange',()=>{if(!document.hidden)refreshSavedLocks()});
...
async function connectFirebase(){if(!window.EstuaryFirebase){firebaseStatusMessage('Firebase is still loading. Try again in a moment.');return}if(firebaseClient)return;try{const input=document.getElementById('firebaseConfig'),raw=input?.value.trim()||JSON.stringify(builtInFirebaseConfig),config=JSON.parse(raw);if(!config.apiKey||!config.authDomain||!config.projectId)throw new Error('Missing required configuration');localStorage.setItem(localKeys.firebaseConfig,JSON.stringify(config));firebaseClient=window.EstuaryFirebase.create(config);firebaseClient.onAuthStateChanged(firebaseClient.auth,onFirebaseUser);firebaseStatusMessage('Firebase connected. Sign in with Google to start syncing.')}catch(error){console.error(error);firebaseStatusMessage('Firebase could not start. Reload Estuary and try again.')}}
...
function calculatedMonthValues(m){const s=data.settings,utilities=m.water+m.electricity+m.gas,kevinFood=m.groceries*s.foodShare/100,megan=m.groceries*(1-s.foodShare/100)+s.meganR1+s.meganR2,expenses=s.fixedExpenses+s.kevinMortgage+utilities+kevinFood-m.cashOffset,income=monthlyIncomeForMonth(m);return{utilities,megan,expenses,income,saved:round(income-expenses-m.spending)}}
function total(m){const values=calculatedMonthValues(m),locked=m.savedLocked!==undefined&&m.savedLocked!==null&&m.savedLocked!=='';return{...values,saved:locked?num(m.savedLocked):values.saved,savedLocked:locked}}
