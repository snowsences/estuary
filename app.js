const localKeys = {
    data: 'estuary-v1',
    displayMode: 'estuary-display-mode',
    firebaseConfig: 'estuary-firebase-config',
    pkce: 'estuary-pkce',
    theme: 'estuary-theme',
  },
  legacyPrefix = ['cream', 'corn'].join('-');
Object.entries(localKeys).forEach(([name, newKey]) => {
  const oldKey =
    name === 'data'
      ? `${legacyPrefix}-v1`
      : `${legacyPrefix}-${name === 'firebaseConfig' ? 'firebase-config' : name === 'displayMode' ? 'display-mode' : name}`;
...
const key = localKeys.data,
  ...
  round = n => Math.round((Number(n) || 0) * 100) / 100,
  maskDigits = value => String(value).replace(/\d/g, '9'),
...
let displayMode = localStorage.getItem(localKeys.displayMode) === 'on',
  ...
  money = n => {
    ...
    return displayMode ? maskDigits(value) : value;
  };
const investmentAccountTypes = ['IRA', '401k', 'Brokerage', 'Savings'];
const defaultInvestmentAccounts = [
  { id: 'ira', name: 'IRA', type: 'IRA', annualGoal: 0, currentTotal: 0, archived: false },
  { id: '401k-1', name: '401(k) 1', type: '401k', yearlyContribution: 0, currentTotal: 0, archived: false },
  { id: '401k-2', name: '401(k) 2', type: '401k', yearlyContribution: 0, currentTotal: 0, archived: false },
  {
    id: 'brokerage',
    name: 'Brokerage · VOO',
    type: 'Brokerage',
...
const starter = {
  ...
  settings: {
    ...
    meganR1: 0,
    meganR2: 0,
    investmentAccounts: defaultInvestmentAccounts.map(account => ({ ...account })),
  },
  dropbox: { appKey: '', filePath: '/Budget.xlsx', accessToken: '', name: '', revision: '', rowMap: {} },
};
const safeText = (value, maxLength) =>
    String(value ?? '')
      .replace(/[\u0000-\u0008\u000B\u000C\u000E-\u001F\u007F]/g, ' ')
      .slice(0, maxLength),
  monthIdPattern = /^(?:19|20|21)\d{2}-(?:0[1-9]|1[0-2])$/,
  safeMonthId = value => (monthIdPattern.test(String(value || '')) ? String(value) : ''),
  safeId = (value, prefix = 'item') => {
    const id = String(value || '');
    return /^[A-Za-z0-9_-]{1,128}$/.test(id) ? id : `${prefix}-${crypto.randomUUID()}`;
  },
  safeDateTime = value => {
    const text = String(value || '');
    return text.length <= 40 && Number.isFinite(Date.parse(text))
      ? new Date(text).toISOString()
      : new Date().toISOString();
  },
  safeNumber = (value, min = -10000000, max = 10000000) => Math.max(min, Math.min(max, num(value)));
const safeDateId = value => {
  const text = String(value || '');
...
function normalizeMonthRecord(value, fallbackId = '') {
  if (!value || typeof value !== 'object' || Array.isArray(value)) return null;
  const id = safeMonthId(value.id) || safeMonthId(fallbackId);
  if (!id) return null;
  const monthIndex = Number(id.slice(5, 7)) - 1,
    record = {
      id,
      label: `${monthLabels[monthIndex]} ${id.slice(0, 4)}`,
      water: safeNumber(value.water),
      electricity: safeNumber(value.electricity),
      gas: safeNumber(value.gas),
      groceries: safeNumber(value.groceries),
      spending: safeNumber(value.spending),
      cashOffset: safeNumber(value.cashOffset),
      notes: safeText(value.notes, 500),
    };
  if (value.savedLocked !== undefined && value.savedLocked !== null && value.savedLocked !== '')
    record.savedLocked = safeNumber(value.savedLocked);
  if (value.savedLockedAt) record.savedLockedAt = safeDateTime(value.savedLockedAt);
  if (Number.isFinite(Number(value.incomePaycheck)))
    record.incomePaycheck = safeNumber(value.incomePaycheck, 0);
  return record;
}
function normalizeExpenseRecord(value, fallbackId = '') {
  if (!value || typeof value !== 'object' || Array.isArray(value)) return null;
  const month = safeMonthId(value.month),
    types = ['Spending', 'Water', 'Electricity', 'Gas', 'Groceries'];
  if (!month || !types.includes(value.type)) return null;
  const legacyDining =
      value.category === 'Restaurants' || (value.category === 'Shopping' && value.subcategory === 'Dining'),
    category = legacyDining ? 'Dining' : value.category,
    categories = ['Groceries', 'Dining', 'Shopping'],
    subcategories = ['', 'Entertainment', 'Travel', 'Health', 'Pets', 'Misc'];
  const entry = {
    id: safeId(value.id || fallbackId, 'expense'),
    type: value.type,
    month,
    original: safeNumber(value.original, 0),
    modifier: safeNumber(value.modifier, 0.9, 1),
    final: safeNumber(value.final, 0),
    comment: safeText(value.comment, 500),
    createdAt: safeDateTime(value.createdAt),
  };
  if (categories.includes(category)) entry.category = category;
  if (legacyDining) entry.subcategory = '';
  else if (subcategories.includes(value.subcategory)) entry.subcategory = value.subcategory;
  if (value.updatedAt) entry.updatedAt = safeDateTime(value.updatedAt);
  return entry;
}
function normalizeExtraIncomeRecord(value, fallbackId = '') {
  if (!value || typeof value !== 'object' || Array.isArray(value)) return null;
  const month = safeMonthId(value.month),
    categories = ['Signup Bonus', 'Gift', 'Misc'];
  if (!month || !categories.includes(value.category)) return null;
  const entry = {
    id: safeId(value.id || fallbackId, 'income'),
    month,
...
function normalizeInvestmentRecord(value, fallbackId = '') {
  if (!value || typeof value !== 'object' || Array.isArray(value)) return null;
  const date = safeDateId(value.date),
    accountId = /^[A-Za-z0-9_-]{1,128}$/.test(String(value.accountId || '')) ? String(value.accountId) : '';
  if (!date || !accountId || value.contributionType !== 'Your contribution') return null;
  const entry = {
    id: safeId(value.id || fallbackId, 'investment'),
    accountId,
    date,
    amount: safeNumber(value.amount, 0),
    contributionType: 'Your contribution',
    description: safeText(value.description, 300),
    createdAt: safeDateTime(value.createdAt),
  };
  if (value.updatedAt) entry.updatedAt = safeDateTime(value.updatedAt);
...
function normalizeInvestmentBalanceRecord(value, fallbackId = '') {
  if (!value || typeof value !== 'object' || Array.isArray(value)) return null;
  const date = safeDateId(value.date),
    accountId = /^[A-Za-z0-9_-]{1,128}$/.test(String(value.accountId || '')) ? String(value.accountId) : '',
    hasTotal = value.total !== undefined && value.total !== null && value.total !== '';
  if (!date || !accountId || !hasTotal) return null;
  const entry = {
    id: safeId(value.id || fallbackId || `${accountId}-${date}`, 'balance'),
    accountId,
    date,
...
function sanitizeAccountSvgMarkup(value) {
  const text = String(value || '').trim();
  if (!text || new TextEncoder().encode(text).length > 100000) return '';
  const documentNode = new DOMParser().parseFromString(text, 'image/svg+xml'),
    root = documentNode.documentElement;
  if (root.localName !== 'svg' || documentNode.querySelector('parsererror')) return '';
  ...
  [...root.querySelectorAll('*')].forEach(element => {
    ...
    [...element.attributes].forEach(attribute => {
      const name = attribute.name.toLowerCase(),
        content = attribute.value.toLowerCase(),
        unsafeUrl = /url\((?!\s*['"]?#)/i.test(attribute.value);
      if (
        name.startsWith('on') ||
        name === 'href' ||
        name === 'xlink:href' ||
  ...
  return new TextEncoder().encode(cleaned).length <= 100000 ? cleaned : '';
}
function normalizeAccountRecord(value, fallbackId = '') {
  if (!value || typeof value !== 'object' || Array.isArray(value)) return null;
  const kinds = ['Credit Card', 'Bank Account'],
    statuses = ['Active', 'Closed'],
    name = safeText(value.name, 80).trim();
  if (!name || !kinds.includes(value.kind)) return null;
  const record = {
    id: safeId(value.id || fallbackId, 'account'),
    kind: value.kind,
    name,
    use: safeText(value.use, 500).trim(),
    signupBonus: safeText(value.signupBonus, 1000).trim(),
    status: statuses.includes(value.status) ? value.status : 'Active',
    imageSvg: sanitizeAccountSvgMarkup(value.imageSvg),
    createdAt: safeDateTime(value.createdAt),
  };
  if (value.updatedAt) record.updatedAt = safeDateTime(value.updatedAt);
  return record;
}
function normalizeInvestmentAccount(value, fallback = {}) {
  if (!value || typeof value !== 'object' || Array.isArray(value)) return null;
  const id = safeId(value.id || fallback.id, 'investment-account'),
    name = safeText(value.name || fallback.name || 'Investment account', 60).trim(),
    inferred = String(value.id || value.name || '')
      .toLowerCase()
      .includes('401')
      ? '401k'
      : String(value.id || value.name || '')
        ...
        : String(value.id || value.name || '')
          ...
          ? 'Savings'
          : 'IRA',
    type = investmentAccountTypes.includes(value.type) ? value.type : inferred;
  if (!name) return null;
  return {
    id,
    name,
    type,
    annualGoal:
      type === 'IRA' || type === 'Brokerage' ? safeNumber(value.annualGoal ?? fallback.annualGoal, 0) : 0,
    yearlyContribution:
      type === '401k'
        ? safeNumber(value.yearlyContribution ?? value.annualGoal ?? fallback.yearlyContribution, 0)
        : 0,
    currentTotal: safeNumber(value.currentTotal ?? fallback.currentTotal, 0),
    archived: value.archived === true,
    createdAt: safeDateTime(value.createdAt || fallback.createdAt),
  };
}
function normalizeSettings(value) {
  ...
  const settings = {
    monthlyIncome: safeNumber(source.monthlyIncome ?? starter.settings.monthlyIncome, 0),
    fixedExpenses: safeNumber(source.fixedExpenses ?? starter.settings.fixedExpenses, 0),
    mortgagePayment,
    kevinMortgage: round(mortgagePayment - meganMortgage),
    foodShare: safeNumber(source.foodShare ?? starter.settings.foodShare, 0, 100),
    meganR1: meganMortgage,
    meganR2: safeNumber(source.meganR2 ?? starter.settings.meganR2),
    financeNotes: safeText(source.financeNotes, 5000),
    financeNotes2: safeText(source.financeNotes2, 5000),
    financeNotes3: safeText(source.financeNotes3, 5000),
    financeNotes4: safeText(source.financeNotes4, 5000),
  };
  if (Array.isArray(source.fixedExpenseItems))
    settings.fixedExpenseItems = source.fixedExpenseItems
      .slice(0, 100)
      .filter(item => item && typeof item === 'object' && !Array.isArray(item))
      .map(item => ({
        id: safeId(item.id, 'fixed'),
        label: safeText(item.label || 'Expense', 80),
        amount: safeNumber(item.amount),
      }));
  if (Array.isArray(source.incomeLevels))
    settings.incomeLevels = source.incomeLevels
      .slice(0, 100)
      .filter(item => item && typeof item === 'object' && !Array.isArray(item) && safeMonthId(item.start))
      .map(item => ({
        id: safeId(item.id, 'income'),
        start: safeMonthId(item.start),
        end: safeMonthId(item.end),
        paycheck: safeNumber(item.paycheck, 0),
      }));
  const accountSource =
    Array.isArray(source.investmentAccounts) && source.investmentAccounts.length
      ? source.investmentAccounts.slice(0, 100)
      : defaultInvestmentAccounts;
  const seen = new Set();
  settings.investmentAccounts = accountSource
    .map((item, index) => normalizeInvestmentAccount(item, defaultInvestmentAccounts[index] || {}))
    .filter(account => account && !seen.has(account.id) && seen.add(account.id));
  if (typeof source.incomeHistoryLocked === 'boolean')
    settings.incomeHistoryLocked = source.incomeHistoryLocked;
  if (safeMonthId(source.meganReminderSeen))
    settings.meganReminderSeen = safeMonthId(source.meganReminderSeen);
  return settings;
}
function normalizeAppData(value) {
  const source = value && typeof value === 'object' && !Array.isArray(value) ? value : {},
    ...
    rawInvestments = Array.isArray(source.investments) ? source.investments.slice(0, 20000) : [],
    investments = rawInvestments.map(entry => normalizeInvestmentRecord(entry)).filter(Boolean),
    balanceMap = new Map();
  if (Array.isArray(source.investmentBalances))
    source.investmentBalances.slice(0, 20000).forEach(value => {
      const entry = normalizeInvestmentBalanceRecord(value);
      if (entry) balanceMap.set(`${entry.accountId}|${entry.date}`, entry);
    });
  rawInvestments.forEach(value => {
    if (value?.currentTotal === undefined || value?.currentTotal === null || value?.currentTotal === '')
      return;
    const entry = normalizeInvestmentBalanceRecord({
      id: `${value.accountId}-${value.date}`,
    ...
    if (entry) {
      const key = `${entry.accountId}|${entry.date}`;
      if (!balanceMap.has(key)) balanceMap.set(key, entry);
    }
  });
  const today = new Date().toISOString().slice(0, 10);
  settings.investmentAccounts.forEach(account => {
    if (
      num(account.currentTotal) <= 0 ||
      [...balanceMap.values()].some(entry => entry.accountId === account.id)
    )
      return;
    const entry = normalizeInvestmentBalanceRecord({
      id: `${account.id}-${today}`,
  ...
  return {
    ...
    entries: Array.isArray(source.entries)
      ? source.entries
          .slice(0, 10000)
          .map(entry => normalizeExpenseRecord(entry))
          .filter(Boolean)
      : [],
    extraIncome: Array.isArray(source.extraIncome)
      ? source.extraIncome
          .slice(0, 10000)
          .map(entry => normalizeExtraIncomeRecord(entry))
          .filter(Boolean)
      : [],
    investments,
    investmentBalances: [...balanceMap.values()],
...
try {
  ...
  console.warn('Ignored invalid local budget data.', error);
}
let data = normalizeAppData(storedData || starter);
const currentYear = new Date().getFullYear();
function ensureYear(year) {
  for (let i = 0; i < 12; i++) {
    const id = `${year}-${String(i + 1).padStart(2, '0')}`;
    if (!data.months.some(month => month.id === id))
      data.months.push({
        id,
...
function renderCore() {
  const m = data.months.find(x => x.id === active),
    ...
    savingsYear = isJanuary ? calendarYear - 1 : calendarYear,
    completedMonthCount = isJanuary ? 12 : today.getMonth(),
    savedMonths = data.months.filter(
      x =>
        Number(x.id.slice(0, 4)) === savingsYear &&
        (isJanuary || Number(x.id.slice(5, 7)) <= completedMonthCount),
    ),
    previousYear = savingsYear - 1,
    previousSavedMonths = data.months.filter(x => Number(x.id.slice(0, 4)) === previousYear),
    savedTotal = savedMonths.reduce((sum, x) => sum + total(x).saved, 0),
    savedAverage = savedMonths.length ? savedTotal / savedMonths.length : 0,
    previousTotal = previousSavedMonths.reduce((sum, x) => sum + total(x).saved, 0),
    previousAverage = previousSavedMonths.length ? previousTotal / previousSavedMonths.length : 0,
    periodLabel = isJanuary ? 'Last Year' : 'This Year',
    comparisonLabel = isJanuary ? String(previousYear) : 'Last year';
  monthPicker.innerHTML = data.months.map(x => `<option value="${x.id}">${x.label}</option>`).join('');
  monthPicker.value = active;
  expenseMonth.innerHTML = monthPicker.innerHTML;
  expenseMonth.value = active;
  metrics.innerHTML = `<article class="metric orange"><p>Spent This Month</p><strong>${money(m.spending)}</strong></article><article class="metric violet"><p>Groceries This Month</p><strong>${money(m.groceries)}</strong></article><article class="metric green"><p>Avg Saved ${periodLabel}</p><strong>${money(savedAverage)}</strong><span>${comparisonLabel}: ${money(previousAverage)}</span></article><article class="metric"><p>Total Saved ${periodLabel}</p><strong>${money(savedTotal)}</strong><span>${comparisonLabel}: ${money(previousTotal)}</span></article>`;
  const years = [...new Set(data.months.map(x => x.id.slice(0, 4)))].sort((a, b) => Number(b) - Number(a));
  if (!years.includes(selectedYear))
    selectedYear = years.includes(String(currentYear)) ? String(currentYear) : years[0];
  yearTimeline.innerHTML = years
    .map(itemYear => {
      const group = data.months.filter(x => x.id.startsWith(itemYear)),
        totalSaved = Math.round(group.reduce((sum, x) => sum + total(x).saved, 0));
      return `<button class="${itemYear === selectedYear ? 'selected' : ''}" data-year-tab="${itemYear}"><strong>${itemYear}</strong><span>Total saved ${money(totalSaved)}</span></button>`;
    })
    .join('');
  monthlyYearTable.innerHTML = monthTable(data.months.filter(x => x.id.startsWith(selectedYear)));
  document
    .querySelectorAll('[data-setting]')
    .forEach(i => (i.value = round(data.settings[i.dataset.setting])));
...
document.addEventListener('change', e => {
  ...
  if (el.dataset.setting) {
    data.settings[el.dataset.setting] = num(el.value);
    if (el.dataset.setting === 'mortgagePayment' || el.dataset.setting === 'meganR1')
      data.settings.kevinMortgage = round(num(data.settings.mortgagePayment) - num(data.settings.meganR1));
    save();
    syncSettings();
    render();
  }
...
const builtInFirebaseConfig = {
    apiKey: 'AIzaSyAmBvHoVCBm7mbOx_v9wPQeq-FwQ0F59eo',
    authDomain: 'cream-corn-8e8ea.firebaseapp.com',
    projectId: 'cream-corn-8e8ea',
    storageBucket: 'cream-corn-8e8ea.firebasestorage.app',
    messagingSenderId: '323631470609',
    appId: '1:323631470609:web:f1547cb37954e4fa0bb9b0',
  },
  ownerUid = 'R9klEiIEcsMZBrJwn12BaScJE8J2';
...
const freshMonth = id => {
  const monthNumber = Number(id.slice(5, 7)) - 1;
  return {
    id,
    label: `${monthLabels[monthNumber]} ${id.slice(0, 4)}`,
    water: 0,
    electricity: 0,
    gas: 0,
    groceries: 0,
...
const investmentSettingsScore = accounts =>
  accounts.length * 100 +
  accounts.reduce((score, account, index) => {
    const original = defaultInvestmentAccounts[index];
    return (
      score +
      (!original ||
...
async function syncInvestmentSettings() {
  if (!firebaseClient || !firebaseUser) return;
  latestLocalInvestmentSettingsUpdate = Date.now();
  try {
    await firebaseClient.setDoc(
      firebaseClient.doc(firebaseClient.db, ...firebasePath('budget'), 'investmentSettings'),
      {
        investmentAccounts: data.settings.investmentAccounts.map(account => ({ ...account })),
        updatedAt: latestLocalInvestmentSettingsUpdate,
      },
    );
  } catch (error) {
    console.error(error);
    firebaseStatusMessage('Could not sync Investment Settings. Your local changes are still saved here.');
  }
}
async function loadInvestmentSettings() {
  if (!firebaseClient || !firebaseUser) return;
  const localAccounts = normalizeSettings({
    investmentAccounts: data.settings.investmentAccounts,
  }).investmentAccounts;
  const investmentRef = firebaseClient.doc(
    firebaseClient.db,
    ...firebasePath('budget'),
    'investmentSettings',
  );
  ...
  if (investmentSnapshot.exists() && Array.isArray(investmentSnapshot.data().investmentAccounts)) {
    ...
    const remoteAccounts = normalizeSettings({
      investmentAccounts: remote.investmentAccounts,
    }).investmentAccounts;
  ...
  const legacyAccounts =
    Array.isArray(legacyRaw) && legacyRaw.length
      ? normalizeSettings({ investmentAccounts: legacyRaw }).investmentAccounts
      : [];
  data.settings.investmentAccounts =
    investmentSettingsScore(localAccounts) >= investmentSettingsScore(legacyAccounts)
      ? localAccounts
      : legacyAccounts;
  save();
  await syncInvestmentSettings();
}
const syncGeneralSettingsOnly = syncSettings;
syncSettings = async () => {
  await syncGeneralSettingsOnly();
  await syncInvestmentSettings();
};
async function syncMonth(month) {
  if (!firebaseClient || !firebaseUser) return;
  try {
    await firebaseClient.setDoc(firebaseClient.doc(firebaseClient.db, ...firebasePath('months'), month.id), {
      ...cleanMonth(month),
...
async function addCloudCashOffset(monthId, amount) {
  if (!firebaseClient || !firebaseUser) return;
  const monthRef = firebaseClient.doc(firebaseClient.db, ...firebasePath('months'), monthId);
  try {
    await firebaseClient.runTransaction(firebaseClient.db, async transaction => {
      ...
      remote.cashOffset = round(remote.cashOffset + amount);
      transaction.set(monthRef, { ...remote, updatedAt: new Date().toISOString() });
    });
  } catch (error) {
    console.error(error);
    firebaseStatusMessage('Could not sync this cash offset yet. It remains saved on this device.');
  }
}
async function syncExtraIncomeRecord(entry) {
  if (!firebaseClient || !firebaseUser) return;
  await firebaseClient.setDoc(
    firebaseClient.doc(firebaseClient.db, ...firebasePath('extraIncome'), entry.id),
    entry,
  );
}
async function addCloudExtraIncome(entry) {
  if (!firebaseClient || !firebaseUser) return;
  const monthRef = firebaseClient.doc(firebaseClient.db, ...firebasePath('months'), entry.month),
    incomeRef = firebaseClient.doc(firebaseClient.db, ...firebasePath('extraIncome'), entry.id);
  try {
    await firebaseClient.runTransaction(firebaseClient.db, async transaction => {
      const snapshot = await transaction.get(monthRef),
        remote = cleanMonth(snapshot.exists() ? snapshot.data() : freshMonth(entry.month));
      remote.spending = round(remote.spending - entry.amount);
      transaction.set(monthRef, { ...remote, updatedAt: new Date().toISOString() });
      transaction.set(incomeRef, entry);
    });
  } catch (error) {
    console.error(error);
    firebaseStatusMessage('Could not sync this extra income yet. It remains saved on this device.');
  }
}
async function updateCloudExtraIncome(previous, entry) {
  if (!firebaseClient || !firebaseUser) return;
  const oldMonthRef = firebaseClient.doc(firebaseClient.db, ...firebasePath('months'), previous.month),
    newMonthRef = firebaseClient.doc(firebaseClient.db, ...firebasePath('months'), entry.month),
    incomeRef = firebaseClient.doc(firebaseClient.db, ...firebasePath('extraIncome'), entry.id);
  try {
    await firebaseClient.runTransaction(firebaseClient.db, async transaction => {
      if (previous.month === entry.month) {
        const snapshot = await transaction.get(oldMonthRef),
          remote = cleanMonth(snapshot.exists() ? snapshot.data() : freshMonth(previous.month));
        remote.spending = round(remote.spending + previous.amount - entry.amount);
        transaction.set(oldMonthRef, { ...remote, updatedAt: new Date().toISOString() });
      } else {
        const oldSnapshot = await transaction.get(oldMonthRef),
          newSnapshot = await transaction.get(newMonthRef),
          oldRemote = cleanMonth(oldSnapshot.exists() ? oldSnapshot.data() : freshMonth(previous.month)),
          newRemote = cleanMonth(newSnapshot.exists() ? newSnapshot.data() : freshMonth(entry.month));
        oldRemote.spending = round(oldRemote.spending + previous.amount);
        newRemote.spending = round(newRemote.spending - entry.amount);
        transaction.set(oldMonthRef, { ...oldRemote, updatedAt: new Date().toISOString() });
        transaction.set(newMonthRef, { ...newRemote, updatedAt: new Date().toISOString() });
      }
      transaction.set(incomeRef, entry);
    });
  } catch (error) {
    console.error(error);
    firebaseStatusMessage('Could not sync this extra income edit yet. It remains saved on this device.');
  }
}
async function deleteCloudExtraIncome(entry) {
  if (!firebaseClient || !firebaseUser) return;
  const monthRef = firebaseClient.doc(firebaseClient.db, ...firebasePath('months'), entry.month),
    incomeRef = firebaseClient.doc(firebaseClient.db, ...firebasePath('extraIncome'), entry.id);
  try {
    ...
    firebaseStatusMessage(
      'Could not delete this extra income from sync yet. It remains removed on this device.',
    );
  }
}
async function syncInvestmentRecord(entry) {
  if (!firebaseClient || !firebaseUser) return;
  try {
    await firebaseClient.setDoc(
      firebaseClient.doc(firebaseClient.db, ...firebasePath('investments'), entry.id),
      entry,
    );
  } catch (error) {
    console.error(error);
    firebaseStatusMessage('Could not sync this investment yet. It remains saved on this device.');
  }
}
async function deleteCloudInvestment(id) {
  if (!firebaseClient || !firebaseUser) return;
  try {
    await firebaseClient.deleteDoc(firebaseClient.doc(firebaseClient.db, ...firebasePath('investments'), id));
  } catch (error) {
    console.error(error);
    firebaseStatusMessage(
      'Could not delete this investment from sync yet. It remains removed on this device.',
    );
  }
}
async function syncInvestmentBalanceRecord(entry) {
  if (!firebaseClient || !firebaseUser) return;
  try {
    await firebaseClient.setDoc(
      firebaseClient.doc(firebaseClient.db, ...firebasePath('investmentBalances'), entry.id),
      entry,
    );
  } catch (error) {
    console.error(error);
    firebaseStatusMessage('Could not sync this balance update yet. It remains saved on this device.');
  }
}
async function migrateLegacyInvestmentBalanceRecord(entry) {
  if (!firebaseClient || !firebaseUser) return;
  const reference = firebaseClient.doc(firebaseClient.db, ...firebasePath('investmentBalances'), entry.id);
  try {
    await firebaseClient.runTransaction(firebaseClient.db, async transaction => {
      const snapshot = await transaction.get(reference);
      if (!snapshot.exists()) transaction.set(reference, entry);
    });
  } catch (error) {
    console.warn('Could not migrate a legacy investment balance.', error);
  }
}
async function syncAccountRecord(entry) {
  if (!firebaseClient || !firebaseUser) return;
  try {
    await firebaseClient.setDoc(
      firebaseClient.doc(firebaseClient.db, ...firebasePath('accounts'), entry.id),
      entry,
    );
  } catch (error) {
    console.error(error);
    firebaseStatusMessage('Could not sync this account yet. It remains saved on this device.');
  }
}
async function deleteCloudAccount(id) {
  if (!firebaseClient || !firebaseUser) return;
  try {
    await firebaseClient.deleteDoc(firebaseClient.doc(firebaseClient.db, ...firebasePath('accounts'), id));
  } catch (error) {
    console.error(error);
    firebaseStatusMessage('Could not delete this account from sync yet. It remains removed on this device.');
  }
}
async function migrateLocalData() {
  const monthsCollection = firebaseClient.collection(firebaseClient.db, ...firebasePath('months')),
  ...
  await Promise.all(
    data.entries.map(entry => {
      ...
      return firebaseClient.setDoc(firebaseClient.doc(firebaseClient.db, ...firebasePath('expenses'), id), {
        ...entry,
        id,
      });
    }),
  );
  await Promise.all(data.extraIncome.map(syncExtraIncomeRecord));
  await Promise.all(data.investments.map(syncInvestmentRecord));
  await Promise.all(data.investmentBalances.map(syncInvestmentBalanceRecord));
  await Promise.all(data.accounts.map(syncAccountRecord));
  return true;
}
function stopFirebaseListeners() {
  firebaseUnsubscribers.forEach(stop => stop());
  firebaseUnsubscribers = [];
}
function fourMonthSavedCutoff() {
  const now = new Date(),
    cutoff = new Date(now.getFullYear(), now.getMonth() - 4, 1);
  return `${cutoff.getFullYear()}-${String(cutoff.getMonth() + 1).padStart(2, '0')}`;
}
function maybeLockSavedMonths() {
  if (!firebaseUser || !firebaseSettingsReady || !firebaseMonthsReady) return false;
  const cutoff = fourMonthSavedCutoff(),
    lockedAt = new Date().toISOString(),
    changed = [];
  data.months.forEach(month => {
    if (
      month.id <= cutoff &&
      (month.savedLocked === undefined || month.savedLocked === null || month.savedLocked === '')
    ) {
      month.savedLocked = calculatedMonthValues(month).saved;
      month.savedLockedAt = lockedAt;
      changed.push(month);
    }
  });
...
function subscribeFirebase() {
  ...
  const investmentBalancesRef = firebaseClient.collection(
    firebaseClient.db,
    ...firebasePath('investmentBalances'),
  );
  const accountsRef = firebaseClient.collection(firebaseClient.db, ...firebasePath('accounts'));
  firebaseUnsubscribers.push(
    firebaseClient.onSnapshot(settingsRef, snapshot => {
          ...
          if (!Object.hasOwn(remoteGeneralSettings, 'mortgagePayment'))
            remoteGeneralSettings.mortgagePayment = round(
              num(remoteGeneralSettings.kevinMortgage ?? data.settings.kevinMortgage) +
                num(remoteGeneralSettings.meganR1 ?? data.settings.meganR1),
            );
  ...
  firebaseUnsubscribers.push(
    firebaseClient.onSnapshot(investmentSettingsRef, snapshot => {
      if (!snapshot.exists() || !Array.isArray(snapshot.data().investmentAccounts)) return;
      const remote = snapshot.data(),
        remoteUpdatedAt = settingsTimestamp(remote.updatedAt);
      if (remoteUpdatedAt && remoteUpdatedAt < latestLocalInvestmentSettingsUpdate) return;
      latestLocalInvestmentSettingsUpdate = Math.max(latestLocalInvestmentSettingsUpdate, remoteUpdatedAt);
      data.settings.investmentAccounts = normalizeSettings({
        investmentAccounts: remote.investmentAccounts,
      }).investmentAccounts;
      save();
      render();
    ...
    firebaseClient.onSnapshot(monthsRef, snapshot => {
      if (!snapshot.empty) {
        ...
        snapshot.docs.forEach(record => {
          ...
          if (!remote) return;
          merged.set(remote.id, { ...(merged.get(remote.id) || freshMonth(remote.id)), ...remote });
        });
        data.months = [...merged.values()].sort((a, b) => a.id.localeCompare(b.id));
        ensureYear(currentYear);
        ensureYear(currentYear + 1);
      }
    ...
    firebaseClient.onSnapshot(investmentsRef, snapshot => {
      ...
      data.investments = snapshot.docs
        .map(record => {
          ...
          if (raw?.currentTotal !== undefined && raw?.currentTotal !== null && raw?.currentTotal !== '') {
            const balance = normalizeInvestmentBalanceRecord({
              id: `${raw.accountId}-${raw.date}`,
              accountId: raw.accountId,
              date: raw.date,
              total: raw.currentTotal,
              createdAt: raw.createdAt,
              updatedAt: raw.updatedAt,
            });
            if (balance) migrated.push(balance);
          }
          return normalizeInvestmentRecord(raw, record.id);
        })
        .filter(Boolean)
        .sort(
          (a, b) =>
      ...
      migrated.forEach(migrateLegacyInvestmentBalanceRecord);
      save();
      render();
    }),
  );
...
const refreshSavedLocks = () => {
  if (maybeLockSavedMonths()) render();
};
document.addEventListener('visibilitychange', () => {
  if (!document.hidden) refreshSavedLocks();
});
window.setInterval(refreshSavedLocks, 60 * 60 * 1000);
function clearPrivateLocalData() {
  localStorage.removeItem(key);
  data = normalizeAppData(starter);
  ensureYear(currentYear);
  ensureYear(currentYear + 1);
  data.months.sort((a, b) => a.id.localeCompare(b.id));
...
async function onFirebaseUser(user) {
  ...
  if (!user) {
    stopFirebaseListeners();
    if (clearPrivateDataAfterAuthChange) {
      clearPrivateLocalData();
      clearPrivateDataAfterAuthChange = false;
    }
    firebaseStatusMessage('Firebase connected. Sign in with Google to sync your private budget.');
    return;
  }
  if (user.uid !== ownerUid) {
    stopFirebaseListeners();
    firebaseStatusMessage('This Google account is not authorized for Estuary.');
    clearPrivateDataAfterAuthChange = true;
  ...
  try {
    ...
    firebaseStatusMessage(
      moved
        ? 'Local budget moved to Firebase and now syncing.'
        : 'Synced as ' + (user.email || 'your Google account') + '.',
    );
  } catch (error) {
    console.error(error);
    firebaseStatusMessage('Firebase connected, but the first sync failed. Check your Firestore rules.');
  }
}
async function connectFirebase() {
  if (!window.EstuaryFirebase) {
    firebaseStatusMessage('Firebase is still loading. Try again in a moment.');
    return;
  }
  if (firebaseClient) return;
  try {
    const input = document.getElementById('firebaseConfig'),
      raw = input?.value.trim() || JSON.stringify(builtInFirebaseConfig),
      config = JSON.parse(raw);
    if (!config.apiKey || !config.authDomain || !config.projectId)
      throw new Error('Missing required configuration');
    localStorage.setItem(localKeys.firebaseConfig, JSON.stringify(config));
    firebaseClient = window.EstuaryFirebase.create(config);
    firebaseClient.onAuthStateChanged(firebaseClient.auth, onFirebaseUser);
    firebaseStatusMessage('Firebase connected. Sign in with Google to start syncing.');
  } catch (error) {
    console.error(error);
    firebaseStatusMessage('Firebase could not start. Reload Estuary and try again.');
  }
}
async function signInFirebase() {
  ...
  try {
    await firebaseClient.signInWithPopup(firebaseClient.auth, new firebaseClient.GoogleAuthProvider());
  } catch (error) {
    console.error(error);
    firebaseStatusMessage(
      'Google sign-in did not finish. Add this GitHub Pages domain to Firebase Authorized domains, then try again.',
    );
  }
}
...
function addCashOffset(monthId) {
  ...
  if (amount > 0) {
    data.months.find(x => x.id === monthId).cashOffset += amount;
    save();
    addCloudCashOffset(monthId, amount);
    notice.textContent = money(amount) + ' added as a cash offset.';
    render();
  }
}
const dialog = document.getElementById('expenseDialog');
addExpense.onclick = () => dialog.showModal();
cancelExpense.onclick = () => dialog.close();
function preview() {
  finalAmount.textContent = money(
    num(expenseAmount.value) * Math.max(0.9, Math.min(1, num(expenseModifier.value))),
  );
}
expenseAmount.oninput = preview;
expenseModifier.oninput = preview;
saveExpense.onclick = () => {
  const original = num(expenseAmount.value),
    modifier = Math.max(0.9, Math.min(1, num(expenseModifier.value)));
  if (original <= 0) return (notice.textContent = 'Enter a positive expense amount.');
  const final = Math.round(original * modifier * 100) / 100,
    type = expenseType.value,
    keyName = type.toLowerCase();
  data.months.find(x => x.id === expenseMonth.value)[keyName] += final;
  ...
  active = expenseMonth.value;
  save();
  dialog.close();
  expenseAmount.value = '';
  notice.textContent = money(final) + ' added to ' + type.toLowerCase() + '.';
  render();
};
document.addEventListener('click', event => {
  const button = event.target.closest('[data-offset-month]');
  if (button) addCashOffset(button.dataset.offsetMonth);
});
...
applyTheme(localStorage.getItem(localKeys.theme) || 'dark');
document.getElementById('firebaseSignIn').onclick = signInFirebase;
document.getElementById('firebaseSignOut').onclick = signOutFirebase;
render();
document.querySelectorAll('[data-expense-type]').forEach(
  button =>
    (button.onclick = () => {
      expenseType.value = button.dataset.expenseType;
      document
        .querySelectorAll('[data-expense-type]')
        .forEach(item => item.classList.toggle('selected', item === button));
      commentField.classList.toggle('hidden', expenseType.value !== 'Spending');
    }),
);
document.querySelectorAll('[data-expense-modifier]').forEach(
  button =>
    (button.onclick = () => {
      expenseModifier.value = button.dataset.expenseModifier;
      document
        .querySelectorAll('[data-expense-modifier]')
        .forEach(item => item.classList.toggle('selected', item === button));
      preview();
    }),
);
...
saveExpense.onclick = () => {
  const original = num(expenseAmount.value),
    modifier = Math.max(0.9, Math.min(1, num(expenseModifier.value)));
  if (original <= 0) return (notice.textContent = 'Enter a positive expense amount.');
  const final = round(original * modifier),
    type = expenseType.value,
    keyName = type.toLowerCase(),
    comment = type === 'Spending' ? expenseComment.value.trim() : '';
  data.months.find(x => x.id === expenseMonth.value)[keyName] = round(
    data.months.find(x => x.id === expenseMonth.value)[keyName] + final,
  );
  data.entries.unshift({
    type,
...
document.addEventListener('click', event => {
  const tab = event.target.closest('[data-year-tab]');
  if (tab) {
    selectedYear = tab.dataset.yearTab;
    render();
  }
});
saveExpense.onclick = () => {
  ...
  const month = data.months.find(x => x.id === entry.month);
  month[keyName] = round(month[keyName] + final);
  if (original > 200 && comment) appendExpenseCommentToNotes(month, entry);
  data.entries.unshift(entry);
  addCloudExpense(entry, keyName);
  syncMonth(month);
  active = entry.month;
  save();
  dialog.close();
...
const renameSetting = (key, label) => {
  const input = document.querySelector(`[data-setting="${key}"]`);
  if (input?.parentNode.firstChild) input.parentNode.firstChild.nodeValue = label;
};
renameSetting('monthlyIncome', 'Kevin monthly income');
renameSetting('kevinMortgage', 'Kevin Mortgage');
renameSetting('meganR1', 'Megan Mortgage');
renameSetting('meganR2', 'Megan Additional');
const appearance = document.createElement('div');
appearance.innerHTML =
  '<p class="eyebrow" style="margin-top:22px">APPEARANCE</p><button class="button ghost" id="themeSettingsToggle"></button>';
document.querySelector('#settings .panel.settings').append(appearance);
const updateThemeButton = () =>
  (themeSettingsToggle.textContent =
    document.documentElement.dataset.theme === 'dark' ? 'Light mode' : 'Dark mode');
updateThemeButton();
themeSettingsToggle.onclick = () => {
...
document.addEventListener('click', event => {
  ...
  if (rangeButton) {
    chartRange = rangeButton.dataset.chartRange;
    renderCharts();
  }
  if (billButton) {
    billMetric = billButton.dataset.billSeries;
    renderCharts();
  }
});
...
function renderCharts() {
  const months = chartMonths(),
    series = {
      water: months.map(month => ({ label: month.label, value: month.water })),
      electricity: months.map(month => ({ label: month.label, value: month.electricity })),
      gas: months.map(month => ({ label: month.label, value: month.gas })),
      food: months.map(month => ({ label: month.label, value: month.groceries })),
      spending: months.map(month => ({ label: month.label, value: month.spending })),
      savings: months.map(month => ({ label: month.label, value: total(month).saved })),
    },
  ...
  chartArea.innerHTML = `<div class="chart-controls"><h2>Cost over time</h2><div class="chart-tabs">${[
    ...
    `<div class="bill-tabs">${[
      ...
      .join('')}</div>`,
  )}${chartCard('Food', series.food, '#e2ad4c')}${chartCard('Spending', series.spending, '#f0845c')}${chartCard('Savings', series.savings, '#b28ad5')}</div><p class="small" style="margin-top:10px">${rangeLabel} · through ${months.at(-1)?.label || 'today'}</p>`;
}
render();
function chartCard(title, points, color, extra = '') {
  const palette = { '#45b8b0': '#35b9b1', '#e2ad4c': '#efb43b', '#f0845c': '#4d96b7', '#b28ad5': '#779fc2' },
    smoothed = rollingAverage(points),
    latest = smoothed.at(-1);
  return `<article class="chart-card"><div class="chart-card-head"><div><h3>${title}</h3><p class="chart-subtitle">3-month rolling average</p></div>${extra}</div><p class="chart-value">Latest: ${money(latest?.value || 0)}</p>${chartSvg(smoothed, palette[color] || color)}</article>`;
}
let notesMonthId = '';
...
function updateNotesCount() {
  notesCount.textContent = `${notesText.value.length} / 500`;
}
function appendExpenseCommentToNotes(month, entry) {
  const item = `• ${money(entry.final)} — ${entry.comment}`,
    existing = String(month.notes || '').trim(),
    next = existing ? `${existing}\n${item}` : item;
  if (next.length <= 500) {
    month.notes = next;
    return true;
  }
  notice.textContent = `${money(entry.final)} added to spending, but its comment could not be added to Notes because that field is full.`;
  return false;
}
function openNotes(monthId) {
  const month = data.months.find(item => item.id === monthId);
  if (!month) return;
  notesMonthId = monthId;
  document.getElementById('notesTitle').textContent = `Notes · ${month.label}`;
  notesText.value = String(month.notes || '').slice(0, 500);
  updateNotesCount();
  notesDialog.showModal();
  notesText.focus();
}
notesText.oninput = () => {
  if (notesText.value.length > 500) notesText.value = notesText.value.slice(0, 500);
  updateNotesCount();
};
document.addEventListener('click', event => {
  const button = event.target.closest('[data-notes-month]');
  if (button) openNotes(button.dataset.notesMonth);
});
document.getElementById('cancelNotes').onclick = () => notesDialog.close();
document.getElementById('saveNotes').onclick = () => {
  const month = data.months.find(item => item.id === notesMonthId);
  if (!month) return;
  month.notes = notesText.value.trim().slice(0, 500);
  save();
  syncMonth(month);
  notesDialog.close();
  render();
...
function trendPoints(values) {
  ...
  if (count < 2) return values.length ? [values[0], values[0]] : [];
  const meanX = (count - 1) / 2,
    meanY = values.reduce((sum, value) => sum + value, 0) / count;
  let numerator = 0,
    denominator = 0;
  ...
  return [intercept, intercept + slope * (count - 1)];
}
function chartSvg(points, color) {
  ...
  const width = 520,
    ...
    trend = trendPoints(values),
    min = Math.min(0, ...values, ...trend),
    max = Math.max(0, ...values, ...trend),
    range = max - min || 1,
    ...
    path = points
      ...
      .join(' '),
    trendPath = trend.length
      ? `M ${x(0).toFixed(1)} ${y(trend[0]).toFixed(1)} L ${x(points.length - 1).toFixed(1)} ${y(trend[1]).toFixed(1)}`
      : '';
  return `<svg class="chart-svg" viewBox="0 0 ${width} ${height}" role="img" aria-label="Three-month rolling average chart with trend line">${yTicks.map(value => `<g><line class="chart-grid-line" x1="${left}" x2="${width - right}" y1="${y(value)}" y2="${y(value)}"/><text class="chart-axis-label" x="${left - 8}" y="${y(value) + 4}" text-anchor="end">${money(value)}</text></g>`).join('')}<line class="chart-axis" x1="${left}" x2="${width - right}" y1="${height - bottom}" y2="${height - bottom}"/><path d="${trendPath}" fill="none" stroke="${color}" stroke-opacity=".5" stroke-width="3" stroke-dasharray="7 6" stroke-linecap="round"/><path d="${path}" fill="none" stroke="${color}" stroke-width="3.5" stroke-linejoin="round" stroke-linecap="round"/>${points.map((point, index) => `<g><circle cx="${x(index)}" cy="${y(point.average)}" r="3.5" fill="${color}"/><circle class="chart-point" data-chart-label="${point.label}" data-chart-value="${money(point.value)}" data-chart-average="${money(point.average)}" cx="${x(index)}" cy="${y(point.average)}" r="13" fill="${color}" fill-opacity=".001"/></g>`).join('')}${xIndexes.map(index => `<g><line class="chart-axis" x1="${x(index)}" x2="${x(index)}" y1="${height - bottom}" y2="${height - bottom + 5}"/><text class="chart-axis-label" x="${x(index)}" y="${height - 15}" text-anchor="middle">${points[index].label.split(' ')[0].slice(0, 3)} ${points[index].label.split(' ').at(-1)}</text></g>`).join('')}</svg>`;
}
function savedTone(value) {
  return value > 4000
    ? 'saved-high'
    : value >= 2500
      ? 'saved-mid'
...
function showChartTooltip(point, event) {
  chartTooltip.innerHTML = `<b>${point.dataset.chartLabel}</b><span>${point.dataset.chartValue}</span><span>3-month average: ${point.dataset.chartAverage}</span>`;
  const gap = 14,
    width = chartTooltip.offsetWidth,
    height = chartTooltip.offsetHeight,
    left = Math.min(window.innerWidth - width - 10, Math.max(10, event.clientX + gap)),
    top = Math.min(window.innerHeight - height - 10, Math.max(10, event.clientY - height - gap));
...
function renderMonthlyTimeline() {
  const timeline = document.getElementById('yearTimeline'),
    ...
    years = [...new Set(data.months.map(month => month.id.slice(0, 4)))].sort(
  ...
  timeline.innerHTML = years
    .map(year => {
      const yearNumber = Number(year),
        months = data.months.filter(month => month.id.startsWith(year)),
        included =
          yearNumber === thisYear
            ? months.filter(month => Number(month.id.slice(5, 7)) < today.getMonth() + 1)
            : months,
        totalSaved = Math.round(included.reduce((sum, month) => sum + total(month).saved, 0)),
        subline =
          yearNumber > thisYear
            ? ''
...
function applyDisplayMode() {
  document.documentElement.classList.toggle('display-mode', displayMode);
  document
    .querySelectorAll('input[type="number"],input[type="date"],input[data-display-original-type]')
    .forEach(input => {
      if (displayMode) {
        if (input.dataset.displayOriginalType) return;
        input.dataset.displayOriginalType = input.type;
        input.dataset.displayRawValue = input.value;
        input.type = 'text';
        input.readOnly = true;
        input.value = maskDigits(input.value);
      } else if (input.dataset.displayOriginalType) {
        const value = input.dataset.displayRawValue;
        input.type = input.dataset.displayOriginalType;
        input.value = value;
        input.readOnly = false;
        delete input.dataset.displayOriginalType;
        delete input.dataset.displayRawValue;
      }
    });
  ...
  let node;
  while ((node = walker.nextNode())) node.nodeValue = maskDigits(node.nodeValue);
}
const displayControl = document.createElement('label');
displayControl.className = 'display-mode-control';
displayControl.innerHTML =
  '<span><b>Display Mode</b><small>Obscure numeric data on this device</small></span><span class="display-switch"><input id="displayModeToggle" type="checkbox" aria-label="Toggle Display Mode"><i></i></span>';
document.querySelector('#settings .panel.settings').append(displayControl);
const displayModeToggle = document.getElementById('displayModeToggle');
displayModeToggle.checked = displayMode;
displayModeToggle.onchange = () => {
...
document.getElementById('dismissInstall').onclick = () => installBanner.classList.add('hidden');
window.addEventListener('appinstalled', () => {
  deferredInstallPrompt = null;
  installBanner.classList.add('hidden');
});
render();
function normalizeIncomeLevels() {
  const settings = data.settings,
    legacyPaycheck = round((num(settings.monthlyIncome) * 12) / 26),
    earliest = data.months.at(0)?.id || '';
  if (!Array.isArray(settings.incomeLevels) || !settings.incomeLevels.length)
    settings.incomeLevels = [{ id: 'legacy-income', start: earliest, end: '', paycheck: legacyPaycheck }];
  settings.incomeLevels = settings.incomeLevels.map((level, index) => ({
    id: level.id || `income-${index}`,
    start: String(level.start || ''),
    end: String(level.end || ''),
    paycheck: num(level.paycheck),
  }));
  const legacy = settings.incomeLevels.find(level => level.id === 'legacy-income');
  if (legacy && earliest && !legacy.start) legacy.start = earliest;
  if (!Number.isFinite(Number(settings.monthlyIncome)))
    settings.monthlyIncome = round((legacyPaycheck * 26) / 12);
}
function incomeLevelForMonth(monthId) {
  normalizeIncomeLevels();
  return [...data.settings.incomeLevels]
    .sort((a, b) => String(b.start).localeCompare(String(a.start)))
    .find(level => (!level.start || level.start <= monthId) && (!level.end || level.end >= monthId));
}
function paycheckForMonth(month) {
  if (Number.isFinite(Number(month.incomePaycheck))) return num(month.incomePaycheck);
  return num(incomeLevelForMonth(month.id)?.paycheck) || round((num(data.settings.monthlyIncome) * 12) / 26);
}
function monthlyIncomeForMonth(month) {
  return round((paycheckForMonth(month) * 26) / 12);
}
function calculatedMonthValues(m) {
  const s = data.settings,
    ...
    income = monthlyIncomeForMonth(m);
  return { utilities, megan, expenses, income, saved: round(income - expenses - m.spending) };
}
function total(m) {
  const values = calculatedMonthValues(m),
    locked = m.savedLocked !== undefined && m.savedLocked !== null && m.savedLocked !== '';
  return { ...values, saved: locked ? num(m.savedLocked) : values.saved, savedLocked: locked };
}
function lockCompletedIncomeMonths() {
  normalizeIncomeLevels();
  if (!data.settings.incomeHistoryLocked) return;
  const current = currentMonthKey(),
    changed = [];
  data.months.forEach(month => {
    if (month.id < current && !Number.isFinite(Number(month.incomePaycheck))) {
      month.incomePaycheck = paycheckForMonth(month);
      changed.push(month);
    }
  });
  if (changed.length) {
...
function syncLegacyMonthlyIncome() {
  const current = data.months.find(month => month.id === currentMonthKey()) || data.months.at(-1);
  if (current) data.settings.monthlyIncome = monthlyIncomeForMonth(current);
}
function setupIncomeHistory() {
  const legacyInput = document.querySelector('[data-setting="monthlyIncome"]'),
    legacyField = legacyInput?.closest('label');
  if (!legacyField || document.getElementById('incomeHistory')) return;
  const host = document.createElement('section');
  host.id = 'incomeHistory';
  host.className = 'income-history';
  legacyField.replaceWith(host);
  host.addEventListener('change', event => {
    const field = event.target.closest('[data-income-field]');
    if (!field) return;
    const level = data.settings.incomeLevels.find(item => item.id === field.dataset.incomeId);
    if (!level) return;
    level[field.dataset.incomeField] =
      field.dataset.incomeField === 'paycheck' ? num(field.value) : field.value;
  ...
  host.addEventListener('click', event => {
    const remove = event.target.closest('[data-remove-income]'),
      add = event.target.closest('#addIncomeLevel'),
      apply = event.target.closest('#applyIncomeHistory');
    if (remove) {
      if (data.settings.incomeLevels.length === 1) return;
      data.settings.incomeLevels = data.settings.incomeLevels.filter(
        level => level.id !== remove.dataset.removeIncome,
      );
      syncLegacyMonthlyIncome();
      save();
      syncSettings();
    ...
    if (add) {
      ...
      data.settings.incomeLevels.push({
        ...
        paycheck: paycheckForMonth({ id: current }),
      });
      save();
      syncSettings();
      render();
    ...
    if (apply) {
      ...
      data.months.forEach(month => {
        if (month.id < current)
          month.incomePaycheck = paycheckForMonth({ ...month, incomePaycheck: undefined });
      });
...
function updateChartControlsSticky() {
  ...
  if (!controls) return;
  controls.classList.toggle('is-stuck', window.scrollY > 0 && controls.getBoundingClientRect().top <= 0);
}
window.addEventListener('scroll', updateChartControlsSticky, { passive: true });
window.addEventListener('resize', updateChartControlsSticky);
const stickyChartRender = renderCharts;
renderCharts = () => {
  stickyChartRender();
  requestAnimationFrame(updateChartControlsSticky);
};
...
function openExpenseEdit(id) {
  ...
  if (!entry) return;
  editingExpenseId = id;
  expenseEditAmount.value = round(entry.original);
  expenseEditModifier.value = round(entry.modifier).toFixed(2);
  expenseEditType.value = entry.type;
  expenseEditMonth.innerHTML = data.months
    .map(month => `<option value="${month.id}">${month.label}</option>`)
    .join('');
  expenseEditMonth.value = entry.month;
  expenseEditComment.value = entry.comment || '';
  expenseEditDialog.showModal();
  expenseEditAmount.focus();
}
document.addEventListener('click', event => {
  const edit = event.target.closest('[data-edit-expense]'),
    remove = event.target.closest('[data-delete-expense]');
  if (edit) openExpenseEdit(edit.dataset.editExpense);
  if (remove) {
    const entry = data.entries.find(item => item.id === remove.dataset.deleteExpense);
    if (!entry || !confirm('Delete this expense?')) return;
    const month = data.months.find(item => item.id === entry.month);
    if (month) month[entry.type.toLowerCase()] = round(month[entry.type.toLowerCase()] - entry.final);
...
expenseEditDialog.addEventListener('click', event => {
  if (event.target === expenseEditDialog) expenseEditDialog.close();
});
document.getElementById('expenseEditForm').addEventListener('submit', event => {
  event.preventDefault();
  const entry = data.entries.find(item => item.id === editingExpenseId),
    original = num(expenseEditAmount.value),
    modifier = Math.max(0.9, Math.min(1, num(expenseEditModifier.value))),
    type = expenseEditType.value,
    monthId = expenseEditMonth.value;
  if (!entry || original <= 0) return;
  const oldMonth = data.months.find(item => item.id === entry.month),
    newMonth = data.months.find(item => item.id === monthId),
    oldKey = entry.type.toLowerCase(),
    newKey = type.toLowerCase(),
    final = round(original * modifier);
  if (oldMonth) oldMonth[oldKey] = round(oldMonth[oldKey] - entry.final);
  if (newMonth) newMonth[newKey] = round(newMonth[newKey] + final);
  Object.assign(entry, {
    original,
    modifier,
    type,
    month: monthId,
    final,
    comment: expenseEditComment.value.trim().slice(0, 500),
    updatedAt: new Date().toISOString(),
  });
  save();
...
function setupBackupTools() {
  ...
  document.getElementById('restoreBackupFile').addEventListener('change', restoreBackup);
}
async function downloadBackup() {
  if (!crypto.subtle) return alert('This browser cannot create encrypted backups.');
  const password = prompt('Choose a password for this backup. You will need it to restore the file.');
  if (!password) return;
  const salt = crypto.getRandomValues(new Uint8Array(16)),
    iv = crypto.getRandomValues(new Uint8Array(12)),
    key = await backupKey(password, salt),
    plain = new TextEncoder().encode(JSON.stringify({ data, exportedAt: new Date().toISOString() })),
    cipher = await crypto.subtle.encrypt({ name: 'AES-GCM', iv }, key, plain),
    payload = {
      format: 'estuary-encrypted-backup',
      version: 1,
      kdf: 'PBKDF2-SHA-256',
      iterations: 150000,
...
async function restoreBackup(event) {
  ...
  try {
    ...
    let restored;
    if (
      payload.format === 'estuary-encrypted-backup' ||
      payload.format === `${legacyPrefix}-encrypted-backup`
    ) {
      const password = prompt('Enter this backup’s password.');
      if (!password) return;
      const key = await backupKey(password, base64ToBytes(payload.salt)),
        plain = await crypto.subtle.decrypt(
          { name: 'AES-GCM', iv: base64ToBytes(payload.iv) },
          key,
          base64ToBytes(payload.ciphertext),
        );
      restored = JSON.parse(new TextDecoder().decode(plain)).data;
    } else restored = payload.data;
    if (
      !restored?.months ||
      !Array.isArray(restored.months) ||
      !Array.isArray(restored.entries) ||
      !confirm('Replace this device’s Estuary data with this backup?')
    )
      return;
    data = normalizeAppData(restored);
    ensureYear(currentYear);
    ensureYear(currentYear + 1);
    data.months.sort((a, b) => a.id.localeCompare(b.id));
    active = data.months.find(month => month.id === currentMonthKey())?.id || data.months.at(-1).id;
      ...
      data.entries.map(entry => {
        ...
        return firebaseClient && firebaseUser
          ? firebaseClient.setDoc(
              firebaseClient.doc(firebaseClient.db, ...firebasePath('expenses'), entry.id),
              entry,
            )
          : Promise.resolve();
      }),
...
function renderUtilityAlert() {
  ...
  if (!alert) return;
  const names = { water: 'Water', electricity: 'Electricity', gas: 'Gas' },
    missing = ['water', 'electricity', 'gas']
      .map(key => [key, data.months.filter(month => utilityValueMissing(month, key))])
...
async function addCloudExpense(entry, keyName, notes) {
  ...
  try {
    await firebaseClient.runTransaction(firebaseClient.db, async transaction => {
      ...
      if (notes !== undefined) remote.notes = notes;
      transaction.set(monthRef, { ...remote, updatedAt: new Date().toISOString() });
    });
...
saveExpense.onclick = () => {
  ...
  const final = round(original * modifier),
    ...
    month = data.months.find(item => item.id === entry.month);
  month[keyName] = round(month[keyName] + final);
  const notesChanged = original > 200 && comment ? appendExpenseCommentToNotes(month, entry) : false;
  data.entries.unshift(entry);
  addCloudExpense(entry, keyName, notesChanged ? month.notes : undefined);
  active = entry.month;
  save();
  dialog.close();
  expenseAmount.value = '';
...
(() => {
  ...
  const updateFinal = () => {
    final.textContent = money(num(amount.value) * Math.max(0.9, Math.min(1, num(modifier.value))));
  };
  const updateType = () => {
  ...
  document.getElementById('expenseCategoryOptions').addEventListener('click', event => {
    ...
    document
      ...
      .forEach(button => button.classList.toggle('selected', button === choice));
    document
      .querySelectorAll('[data-shopping]')
      .forEach((button, index) => button.classList.toggle('selected', index === 0));
    modifier.value = category === 'Dining' ? '0.97' : '0.95';
    document
      .querySelectorAll('[data-new-modifier]')
      .forEach(button => button.classList.toggle('selected', button.dataset.newModifier === modifier.value));
    updateType();
    updateFinal();
  });
  ...
  saveExpense.onclick = () => {
    ...
    const appliedModifier = Math.max(0.9, Math.min(1, num(modifier.value)));
    if (original <= 0) return (notice.textContent = 'Enter a positive expense amount.');
    ...
    const entry = {
      ...
      type,
      category,
      subcategory: category === 'Groceries' || category === 'Dining' ? '' : subcategory,
      month: month.value,
      original,
    ...
    const notesChanged =
      original > 200 && entryComment ? appendExpenseCommentToNotes(targetMonth, entry) : false;
    data.entries.unshift(entry);
...
(() => {
      ...
      .map(month => {
        const dining = round(
          data.entries
            .filter(
              entry =>
                entry.month === month.id && entry.category === 'Shopping' && entry.subcategory === 'Dining',
            )
        ...
        return {
          ...
          expenses: round(
            Math.max(
              0,
              num(data.settings.fixedExpenses) +
                num(data.settings.kevinMortgage) +
                num(month.water) +
                num(month.electricity) +
                num(month.gas) -
                num(month.cashOffset),
            ),
          ),
  ...
  function categoryBarsSvg(months) {
    ...
    const maxValue = Math.max(
      1,
      ...months.flatMap(month => [month.expenses, month.groceries + month.dining, month.spendingOther]),
    );
    ...
    const barWidth = Math.min(20, Math.max(10, groupWidth * 0.22));
    const foodGap = 2;
    const monthLabel = month =>
      `${monthLabels[Number(month.id.slice(5, 7)) - 1].slice(0, 3)} '${month.id.slice(2, 4)}`;
    const bars = months
      .map((month, index) => {
        const center = left + groupWidth * index + groupWidth / 2;
        const expenseX = center - barWidth * 1.65;
        const foodX = center - barWidth / 2;
        const spendingX = center + barWidth * 0.65;
...
(() => {
  ...
  function applySvgIcons() {
    ...
    document.querySelectorAll('.notes-button').forEach(button => {
      button.innerHTML = uiIcons.notes;
    });
    document.querySelectorAll('.history-icon-button[data-edit-expense]').forEach(button => {
      button.innerHTML = uiIcons.edit;
    });
    document.querySelectorAll('.history-icon-button[data-delete-expense]').forEach(button => {
      button.innerHTML = uiIcons.trash;
    });
  }
...
(() => {
  ...
  const updateType = () => {
    ...
    document
      .getElementById('expenseEditShoppingSuboptions')
      .classList.toggle('hidden', category !== 'Shopping');
  ...
  const selectButtons = (selector, value) =>
    document
      .querySelectorAll(selector)
      .forEach(button =>
        button.classList.toggle(
          'selected',
          button.dataset.editCategory === value ||
            button.dataset.editBill === value ||
            button.dataset.editShopping === value ||
            button.dataset.editModifier === String(value),
        ),
      );
  const setCategory = (nextCategory, nextSubcategory) => {
    category = nextCategory;
    subcategory =
      nextSubcategory || (category === 'Bills' ? 'Water' : category === 'Shopping' ? 'Entertainment' : '');
  ...
  const loadEntry = entry => {
    ...
    const inferredCategory =
      entry.category ||
      (entry.type === 'Groceries'
        ? 'Groceries'
        : ['Water', 'Electricity', 'Gas'].includes(entry.type)
          ? 'Bills'
          : 'Shopping');
    const inferredSubcategory =
      entry.subcategory ||
      (inferredCategory === 'Bills' ? entry.type : inferredCategory === 'Shopping' ? 'Entertainment' : '');
  ...
  form.addEventListener('submit', event => {
    ...
    Object.assign(entry, {
      ...
      month: month.value,
      final: newFinal,
      comment: newType === 'Spending' ? comment.value.trim().slice(0, 500) : '',
      updatedAt: new Date().toISOString(),
    });
    save();
    [...new Set([oldMonth, newMonth].filter(Boolean))].forEach(syncMonth);
...
(() => {
  ...
  const decorateEditIcons = () =>
    document.querySelectorAll('.history-icon-button[data-edit-expense]').forEach(button => {
      button.innerHTML = actionEditSvg;
    });
  ...
  renderCharts = () => {
    ...
    if (subtitle)
      subtitle.textContent =
        chartRange === 'all' ? 'All time' : `${chartRange} year${chartRange === '1' ? '' : 's'}`;
  };
  renderCharts();
  renderHistory();
})();
...
(() => {
  ...
  gate.id = 'authGate';
  gate.innerHTML =
    '<section class="auth-card" aria-labelledby="authGateTitle"><h1 id="authGateTitle">Estuary</h1><p>Sign in with your authorized Google account to view your private budget.</p><button class="button primary" id="authGateSignIn">Sign in with Google</button><p id="authGateStatus" aria-live="polite">Checking sign-in…</p></section>';
  ...
  onFirebaseUser = async user => {
    ...
    if (!isOwner)
      lockBudget(
        user ? 'This Google account is not authorized for Estuary.' : 'Sign in with Google to continue.',
      );
...
(() => {
  ...
  if (!localStorage.getItem(localKeys.theme)) applyTheme('light');
  ...
  chartCard = function (title, points, color, extra = '') {
    ...
    const average = points.length
      ? points.reduce((sum, point) => sum + num(point.value), 0) / points.length
      : 0;
  ...
  const renderCategoryBarsWithoutExpenses = () => {
    ...
    if (!months.length) {
      chart.innerHTML =
        '<div class="chart-card-head"><div><h3>Spending by category</h3></div></div><div class="chart-empty">No monthly data yet.</div>';
      chartArea.append(chart);
      return;
    }
    const width = 1040,
      ...
      maxValue = Math.max(
        1,
        ...months.flatMap(month => [month.groceries + month.dining, month.spendingOther]),
      ),
      tickStep = Math.max(100, Math.ceil(maxValue / 4 / 100) * 100),
      topTick = tickStep * 4,
      y = value => top + plotHeight - (value / topTick) * plotHeight,
      groupWidth = plotWidth / months.length,
      barWidth = Math.min(28, Math.max(12, groupWidth * 0.3)),
      foodGap = 2,
      monthLabel = month => ({
        name: monthLabels[Number(month.id.slice(5, 7)) - 1].slice(0, 3),
        year: month.id.slice(2, 4),
      });
    const bars = months
      .map((month, index) => {
        const center = left + groupWidth * index + groupWidth / 2,
          foodX = center - barWidth - 3,
          spendingX = center + 3,
          groceryY = y(month.groceries),
          diningY = y(month.groceries + month.dining),
          groceryHeight = Math.max(0, top + plotHeight - groceryY),
          diningHeight = Math.max(0, top + plotHeight - diningY),
          spendingHeight = Math.max(0, top + plotHeight - y(month.spendingOther)),
          diningBarHeight = Math.max(0, diningHeight - (month.dining && month.groceries ? foodGap : 0)),
          label = monthLabel(month);
        return `<g class="category-bar-group"><rect class="category-bar" x="${foodX}" y="${groceryY}" width="${barWidth}" height="${groceryHeight}" fill="${categoryPalette.groceries}"><title>${month.label}\nGroceries: ${money(month.groceries)}</title></rect>${month.dining ? `<rect class="category-bar" x="${foodX}" y="${diningY}" width="${barWidth}" height="${diningBarHeight}" fill="${categoryPalette.dining}"><title>${month.label}\nDining: ${money(month.dining)}</title></rect>` : ''}<rect class="category-bar" x="${spendingX}" y="${y(month.spendingOther)}" width="${barWidth}" height="${spendingHeight}" fill="${categoryPalette.spending}"><title>${month.label}\nSpending: ${money(month.spendingOther)}</title></rect><text class="chart-axis-label category-bar-month" x="${center}" y="${height - 31}" text-anchor="middle"><tspan x="${center}">${label.name}</tspan><tspan x="${center}" dy="13">${label.year}</tspan></text></g>`;
      })
      .join('');
    const ticks = [0, 1, 2, 3, 4]
      .map(index => {
  ...
  renderCharts = () => {
    priorChartRender();
    renderCategoryBarsWithoutExpenses();
  };

  let fixedItemsNeedSync = false;
  const fixedItemTotal = () =>
    round((data.settings.fixedExpenseItems || []).reduce((sum, item) => sum + num(item.amount), 0));
  const normalizeFixedItems = () => {
    if (!Array.isArray(data.settings.fixedExpenseItems) || !data.settings.fixedExpenseItems.length) {
      data.settings.fixedExpenseItems = [
        {
          id: 'base-fixed-expense',
          label: 'Fixed monthly expenses',
          amount: num(data.settings.fixedExpenses),
        },
      ];
      fixedItemsNeedSync = true;
    }
    data.settings.fixedExpenseItems = data.settings.fixedExpenseItems.map((item, index) => ({
      id: item.id || `fixed-${index}`,
      label: String(item.label || 'Expense'),
  ...
  const setupExpenseSettingsTabs = () => {
    ...
    normalizeFixedItems();
    if (fixedItemsNeedSync && firebaseUser) {
      save();
      syncSettings();
      fixedItemsNeedSync = false;
    }
    let host = document.getElementById('expenseSettingsTabs');
    if (!host) {
      const fields = ['mortgagePayment', 'meganR1', 'meganR2', 'kevinMortgage', 'fixedExpenses', 'foodShare']
        .map(key => document.querySelector(`[data-setting="${key}"]`)?.closest('label'))
        .filter(Boolean);
      ...
      host.innerHTML =
        '<div class="settings-tab-list" role="tablist"><button type="button" class="settings-tab active" data-expense-settings-tab="one" role="tab" aria-selected="true">Expenses 1</button><button type="button" class="settings-tab" data-expense-settings-tab="two" role="tab" aria-selected="false">Static Values</button></div><div class="settings-tab-panel" data-expense-settings-panel="one"></div><div class="settings-tab-panel" data-expense-settings-panel="two" hidden></div>';
      ...
      const kevinHelper = document.createElement('p');
      kevinHelper.id = 'kevinMortgageHelp';
      kevinHelper.className = 'small';
      kevinHelper.textContent = 'Calculated as Mortgage Payment minus Megan Mortgage.';
      kevinInput.closest('label').append(kevinHelper);
      fixedInput.disabled = true;
      fixedInput.setAttribute('aria-describedby', 'fixedExpensesHelp');
      const helper = document.createElement('p');
      helper.id = 'fixedExpensesHelp';
      helper.className = 'small';
      helper.textContent = 'Calculated from Static Values.';
      fixedInput.closest('label').append(helper);
      host.addEventListener('click', event => {
        const tab = event.target.closest('[data-expense-settings-tab]');
        if (tab) {
          const choice = tab.dataset.expenseSettingsTab;
          host.querySelectorAll('[data-expense-settings-tab]').forEach(button => {
            const active = button === tab;
            button.classList.toggle('active', active);
            button.setAttribute('aria-selected', String(active));
          });
          host
            .querySelectorAll('[data-expense-settings-panel]')
            .forEach(panel => (panel.hidden = panel.dataset.expenseSettingsPanel !== choice));
          return;
        }
        if (event.target.closest('[data-add-fixed-expense]')) {
          data.settings.fixedExpenseItems.push({
        ...
        if (remove) {
          data.settings.fixedExpenseItems = data.settings.fixedExpenseItems.filter(
            item => item.id !== remove.dataset.removeFixedExpense,
          );
          data.settings.fixedExpenses = fixedItemTotal();
          save();
          syncSettings();
      ...
      host.addEventListener('change', event => {
        const input = event.target.closest('[data-fixed-expense-field]');
        if (!input) return;
        const item = data.settings.fixedExpenseItems.find(entry => entry.id === input.dataset.fixedExpenseId);
        if (!item) return;
        item[input.dataset.fixedExpenseField] =
          input.dataset.fixedExpenseField === 'amount' ? num(input.value) : input.value.trim();
        data.settings.fixedExpenses = fixedItemTotal();
        save();
        syncSettings();
        render();
    ...
    financeNotesInputs.forEach((input, index) => {
      if (document.activeElement !== input) input.value = data.settings[financeNotesKeys[index]] || '';
    });
    requestAnimationFrame(() => financeNotesInputs.forEach(input => input._resizeToContent?.()));
...
(() => {
  ...
  let feedbackTimer = 0;
  const feedbackTargets = type =>
    type === 'Spending' ? [0, 2, 3] : type === 'Groceries' ? [1, 2, 3] : [2, 3];
  ...
  if (addButton && originalAddExpense)
    addButton.onclick = event => {
      const original = num(document.getElementById('expenseAmount')?.value);
      const modifier = Math.max(0.9, Math.min(1, num(document.getElementById('expenseModifier')?.value)));
...
(() => {
  ...
  chartSvg = (points, color) => {
    ...
    const values = points.map(point => point.average),
      trend = trendPoints(values);
    const min = Math.min(0, ...values, ...trend),
      max = Math.max(0, ...values, ...trend),
      range = max - min || 1;
  ...
  const organizeSettingsPanels = () => {
    ...
    if (syncDescription)
      syncDescription.textContent =
        'Sign in with your authorized Google account to keep Estuary in sync across your devices.';
    ...
    if (!backup) {
      ...
      backup.append(
        panelHeading(
          'BACKUP & RESTORE',
          'Backup & Restore',
          'Download an encrypted backup or restore one when needed.',
        ),
      );
    }
    ...
    if (expenseTabs && incomeHistory) {
      ...
      if (!divider) {
        ...
        divider.className = 'budget-settings-divider';
      }
...
(() => {
  renderIncomeLevels = (message = '') => {
    ...
    host.innerHTML = `<p class="eyebrow">INCOME HISTORY</p><h2>Kevin income levels</h2><div class="income-level-head"><span>Biweekly Paycheck Amount</span><span>From</span><span>Through</span><span></span></div><div>${levels
      .map((level, index) => {
...
(() => {
  ...
  const renderInsights = () => {
    if (!insights) return;
    const current = currentMonthKey();
    const completed = data.months
      .filter(month => month.id < current)
    ...
    if (previousThree.length === 3) {
      const average =
        previousThree.reduce((sum, month) => sum + num(month.spending), 0) / previousThree.length;
    ...
    const sameMonthLastYear = data.months.find(
      month => month.id === `${Number(latest.id.slice(0, 4)) - 1}-${latest.id.slice(5)}`,
    );
    if (sameMonthLastYear && num(sameMonthLastYear.groceries) > 0) {
      const change = round(
        ((num(latest.groceries) - num(sameMonthLastYear.groceries)) / num(sameMonthLastYear.groceries)) * 100,
      );
      if (change)
        messages.push(
    ...
    const savedToDate = data.months
      .filter(month => month.id.startsWith(`${year}-`) && month.id.slice(5) <= cutoff)
      .reduce((sum, month) => sum + total(month).saved, 0);
    const savedLastYear = data.months
      .filter(month => month.id.startsWith(`${year - 1}-`) && month.id.slice(5) <= cutoff)
      .reduce((sum, month) => sum + total(month).saved, 0);
    ...
    insights.innerHTML = messages
      .slice(0, 3)
      .map(message => `<p class="overview-insight">${message}</p>`)
      .join('');
...
(() => {
  ...
  installPanel.innerHTML =
    '<p class="eyebrow">INSTALL APP</p><h2>Estuary on this device</h2><p class="small" id="installSettingsStatus">Chrome will make installation available when this device is eligible.</p>';
  if (installBanner) {
    installBanner.classList.remove('hidden');
  ...
  if (installButton)
    installButton.onclick = async event => {
      await originalInstallClick?.(event);
      placeInstallPanel();
      updateInstallStatus();
    };
  render();
...
(() => {
  ...
  window.addIncomeLevel = event => {
    ...
    data.settings.incomeLevels.push({
      ...
      paycheck: current ? num(current.paycheck) : round((num(data.settings.monthlyIncome) * 12) / 26),
    });
    persistIncomeLevels();
    const notice = document.querySelector('.income-history-notice');
    if (notice) notice.textContent = 'New income level added. Set its paycheck amount and start month.';
  ...
  renderIncomeLevels = (message = '') => {
    ...
    host.innerHTML = `<p class="eyebrow">INCOME HISTORY</p><h2>Kevin income levels</h2><div class="income-level-head"><span>Biweekly Check Amt</span><span>From</span><span>Through</span><span></span></div><div>${levels
      .map((level, index) => {
        const current = index === 0;
...
(() => {
  ...
  window.formatIncomeMonthInput = input => {
  ...
  const persistIncomeTimeline = () => {
    syncLegacyMonthlyIncome();
    save();
    syncSettings();
    render();
  };
  const timelineNotice = text => {
    const notice = document.querySelector('.income-history-notice');
    if (notice) notice.textContent = text;
  };
  window.updateIncomeTimeline = (event, id, field) => {
    event?.stopPropagation();
    const levels = incomeTimeline(),
      index = levels.findIndex(level => level.id === id),
      level = levels[index];
    if (!level) return;
    if (field === 'paycheck') level.paycheck = num(event.target.value);
    else {
      const value = parseIncomeMonth(event.target.value),
        above = levels[index - 1],
        below = levels[index + 1];
      if (!value) {
        render();
        timelineNotice('Enter the month as MM/YYYY.');
        return;
      }
      if ((above && value >= above.start) || (below && value <= below.start)) {
        render();
        timelineNotice('Choose a start month between the adjacent income levels.');
        return;
      }
      level.start = value;
    }
    incomeTimeline();
    persistIncomeTimeline();
  };
  window.addPastIncome = event => {
    event?.preventDefault();
    event?.stopPropagation();
    const levels = incomeTimeline(),
      oldest = levels.at(-1);
    ...
    incomeTimeline();
    persistIncomeTimeline();
    timelineNotice('Past income level added. Set its start month and paycheck amount.');
  };
  window.addIncomeRaise = event => {
    event?.preventDefault();
    event?.stopPropagation();
    const levels = incomeTimeline(),
      current = levels[0],
      start = currentMonthKey();
    if (current && start <= current.start) {
      timelineNotice('The current income level already starts this month.');
      return;
    }
    data.settings.incomeLevels.push({
      id: `income-${Date.now()}`,
      ...
      paycheck: num(current?.paycheck),
    });
    incomeTimeline();
    persistIncomeTimeline();
    timelineNotice('New current income level added. Update its paycheck amount.');
  };
  window.removeIncomeTimeline = (event, id) => {
    event?.preventDefault();
    event?.stopPropagation();
    if (data.settings.incomeLevels.length === 1) return;
    data.settings.incomeLevels = data.settings.incomeLevels.filter(level => level.id !== id);
    incomeTimeline();
    persistIncomeTimeline();
  };
  renderIncomeLevels = (message = '') => {
  ...
  document.addEventListener('input', event => {
    if (event.target.matches('[data-income-format="month"]')) window.formatIncomeMonthInput(event.target);
  });
  document.addEventListener('change', event => {
    const input = event.target.closest('[data-income-action="update"]');
    if (input) window.updateIncomeTimeline(event, input.dataset.incomeId, input.dataset.incomeField);
  });
  document.addEventListener('click', event => {
    const control = event.target.closest('[data-income-action]');
    if (!control || control.matches('input')) return;
    const action = control.dataset.incomeAction;
    if (action === 'remove') window.removeIncomeTimeline(event, control.dataset.incomeId);
    if (action === 'add-past') window.addPastIncome(event);
    if (action === 'add-raise') window.addIncomeRaise(event);
    if (action === 'apply') window.applyIncomeRanges(event);
  });
  render();
})();
...
(() => {
  ...
  const categoryMonthsWithTravel = () =>
    chartMonths().map(month => {
      const entryTotal = subcategory =>
        round(
          data.entries
            .filter(
              entry =>
                entry.month === month.id &&
      ...
      return {
        ...
        dining,
        travel,
        spendingOther: round(Math.max(0, num(month.spending) - dining - travel)),
      };
    });
  const renderCategoryBarsWithTravel = () => {
    ...
    const maxValue = Math.max(
      1,
      ...months.flatMap(month => [month.groceries + month.dining, month.spendingOther, month.travel]),
    );
    const tickStep = Math.max(100, Math.ceil(maxValue / 4 / 100) * 100),
      topTick = tickStep * 4;
    const y = value => top + plotHeight - (value / topTick) * plotHeight,
    ...
    const bars = months
      .map((month, index) => {
        const center = left + groupWidth * index + groupWidth / 2,
          foodX = center - (barWidth * 1.5 + gap),
          spendingX = foodX + barWidth + gap,
          travelX = spendingX + barWidth + gap;
        const groceryY = y(month.groceries),
          diningY = y(month.groceries + month.dining),
          groceryHeight = Math.max(0, top + plotHeight - groceryY),
          diningHeight = Math.max(0, top + plotHeight - diningY),
          spendingHeight = Math.max(0, top + plotHeight - y(month.spendingOther)),
          travelHeight = Math.max(0, top + plotHeight - y(month.travel)),
          diningBarHeight = Math.max(0, diningHeight - (month.dining && month.groceries ? foodGap : 0)),
          label = monthLabel(month);
...
(() => {
  ...
  const setIncomeCategory = category => {
    ...
    document
      .querySelectorAll('[data-extra-income-category]')
      .forEach(button =>
        button.classList.toggle('selected', button.dataset.extraIncomeCategory === category),
      );
  };
  const openExtraIncome = (monthId = currentMonthKey(), entryId = '') => {
    ...
    extraIncomeMonth.innerHTML = data.months
      .map(month => `<option value="${month.id}">${month.label}</option>`)
      .join('');
    const selectedMonth = entry?.month || monthId;
    extraIncomeMonth.value = data.months.some(month => month.id === selectedMonth)
      ? selectedMonth
      : active || data.months.at(-1)?.id || '';
    ...
    extraIncomeDialogDescription.textContent = entry
      ? 'Changes to the amount or month will automatically update Spending.'
      : 'This amount will be subtracted from Spending for the selected month.';
  ...
  const setupHistoryTabs = () => {
    ...
    controls.innerHTML =
      '<div class="history-view-tabs" role="tablist" aria-label="History type"><button type="button" class="history-view-tab active" data-history-view="expenses" role="tab" aria-selected="true">Expenses</button><button type="button" class="history-view-tab" data-history-view="income" role="tab" aria-selected="false">Extra Income</button></div>';
  ...
  const renderExtraIncomeHistory = () => {
    const legacyEntries = data.months
      .filter(month => num(month.cashOffset) > 0)
      .map(month => ({
        id: `legacy-${month.id}`,
        month: month.id,
        amount: num(month.cashOffset),
        category: 'Misc',
        createdAt: `${month.id}-01T00:00:00.000Z`,
        legacy: true,
      }));
    const sorted = [...data.extraIncome, ...legacyEntries].sort((a, b) =>
      String(b.createdAt).localeCompare(String(a.createdAt)),
    );
  ...
  openNotes = monthId => {
    ...
    const rows = [
      ...entries.map(
        entry =>
          `<div><span>${entry.category}</span><strong>+${money(entry.amount)}</strong>${entry.description ? `<small>${escapeHtml(entry.description)}</small>` : ''}</div>`,
      ),
      ...(legacyAmount > 0
        ? [`<div><span>Legacy extra income</span><strong>+${money(legacyAmount)}</strong></div>`]
        : []),
    ];
  ...
  extraIncomeDialog.addEventListener('click', event => {
    if (event.target === extraIncomeDialog) extraIncomeDialog.close();
  });
  extraIncomeForm.addEventListener('submit', event => {
    ...
    const existing = editingExtraIncomeId
      ? data.extraIncome.find(item => item.id === editingExtraIncomeId)
      : null;
    const entry = normalizeExtraIncomeRecord({
      id: existing?.id || crypto.randomUUID(),
      amount,
      month: month.id,
      category: extraIncomeCategory.value,
      description: extraIncomeDescription.value.trim(),
    ...
    notice.textContent = existing
      ...
      : `${money(entry.amount)} in extra income was subtracted from ${month.label} spending.`;
    render();
  });
  document.addEventListener('click', event => {
    ...
    if (!entry || !confirm('Delete this extra income entry? Its amount will be added back to Spending.'))
      return;
    const month = data.months.find(item => item.id === entry.month);
  ...
  afterRender(() => {
    document.querySelectorAll('[data-notes-month]').forEach(button => {
      ...
      const hasExtraIncome =
        incomeForMonth(monthId).length > 0 ||
        num(data.months.find(month => month.id === monthId)?.cashOffset) > 0;
      if (hasExtraIncome) button.classList.add('has-notes');
      button.title =
        hasExtraIncome || data.months.find(month => month.id === monthId)?.notes
          ? 'View notes and extra income'
          : 'Add notes';
    });
    syncHeaderAction();
  });
...
(() => {
  ...
  const contributions = () =>
    data.investments.filter(entry => entry.contributionType === 'Your contribution' && entry.amount > 0);
  ...
  const inferred401kMonths = year => {
    const now = new Date(),
      current = now.getFullYear(),
      numeric = Number(year);
    return numeric < current ? 12 : numeric > current ? 0 : now.getMonth() + 1;
  };
  const accountYearContribution = (account, year) =>
    isSavings(account)
      ? 0
      : is401k(account)
        ? round((num(account.yearlyContribution) / 12) * inferred401kMonths(year))
        : round(
            contributions()
              .filter(entry => entry.accountId === account.id && entry.date.startsWith(year))
              .reduce((sum, entry) => sum + entry.amount, 0),
          );
  const contributionPoints = (account, year) =>
    Array.from({ length: 12 }, (_, index) => {
      const month = `${year}-${String(index + 1).padStart(2, '0')}`,
        available = !is401k(account) || index < inferred401kMonths(year),
        value =
          is401k(account) && available
            ? round(num(account.yearlyContribution) / 12)
            : is401k(account)
              ? 0
              : round(
                  contributions()
                    .filter(entry => entry.accountId === account.id && entry.date.startsWith(month))
                    .reduce((sum, entry) => sum + entry.amount, 0),
                );
      return { label: `${monthLabels[index].slice(0, 3)} ${year}`, value };
    });
  const balancePoints = account => {
    const monthly = new Map();
    balances(account.id).forEach(entry => monthly.set(entry.date.slice(0, 7), entry));
    return [...monthly.entries()].map(([month, entry]) => ({
  ...
  const investmentLineChart = (points, color, label, emptyText) => {
    ...
    const width = 520,
      ...
      x = index => (points.length === 1 ? left : left + (index * plotWidth) / (points.length - 1)),
  ...
  const pieStyle = (values, colorIndexes = values.map((_, index) => index)) => {
    ...
    return `conic-gradient(${values
      .map((value, index) => {
        const from = start;
        start += (value / total) * 100;
        return `${colorAt(colorIndexes[index])} ${from}% ${start}%`;
      })
      .join(',')})`;
  };
  const pieCard = (
  ...
  const selectInvestmentAccount = id => {
    ...
    document
      .querySelectorAll('[data-investment-account]')
      .forEach(button => button.classList.toggle('selected', button.dataset.investmentAccount === id));
  };
  const eligibleContributionAccounts = () =>
    activeAccounts().filter(account => !is401k(account) && !isSavings(account));
  const renderAccountOptions = () => {
    const host = document.getElementById('investmentAccountOptions'),
      eligible = eligibleContributionAccounts();
    host.innerHTML = eligible
      .map(
        account =>
          `<button type="button" class="choice-option" data-investment-account="${account.id}">${escapeHtml(account.name)}</button>`,
      )
      .join('');
    return eligible;
  };
  const openInvestment = (entryId = '') => {
    const entry = entryId ? data.investments.find(item => item.id === entryId) : null,
      eligible = renderAccountOptions();
    if (!entry && !eligible.length) {
      alert('Add an active IRA or Brokerage account in Investment Settings first.');
      return;
    }
    ...
    selectInvestmentAccount(entry?.accountId || eligible[0]?.id || '');
  ...
  const openTotals = () => {
    ...
    if (!accounts.length) {
      alert('Add an investment account first.');
      return;
    }
    totalsFields.innerHTML = accounts
      .map(
        account =>
          `<label><span>${escapeHtml(account.name)}<small>${escapeHtml(account.type)}</small></span><input type="number" min="0" step="0.01" inputmode="decimal" data-balance-account="${account.id}" placeholder="${round(currentAccountTotal(account))}" aria-label="Current total for ${escapeHtml(account.name)}"></label>`,
      )
      .join('');
    totalsDialog.showModal();
    requestAnimationFrame(() => totalsFields.querySelector('input')?.focus());
  };
  ...
  const settingsRow = (account, index, total) => {
    const goalField = is401k(account) ? 'yearlyContribution' : 'annualGoal',
      goalLabel = is401k(account) ? 'Yearly Contribution' : 'Yearly Goal',
      goalValue = round(account[goalField]),
      name = escapeHtml(account.name),
      goalControl = isSavings(account)
        ? '<div class="investment-settings-goal-spacer" aria-hidden="true"></div>'
        : `<label>${goalLabel}<input type="number" min="0" step="0.01" inputmode="decimal" value="${goalValue}" data-investment-setting="${goalField}" data-investment-account-id="${account.id}"></label>`;
    return `<div class="investment-settings-row${account.archived ? ' is-archived' : ''}"><label>Account name<input type="text" maxlength="60" value="${name}" data-investment-setting="name" data-investment-account-id="${account.id}"></label><label>Type<select data-investment-setting="type" data-investment-account-id="${account.id}">${investmentAccountTypes.map(type => `<option${account.type === type ? ' selected' : ''}>${type}</option>`).join('')}</select></label>${goalControl}<div class="investment-settings-actions"><button type="button" class="button ghost investment-order-button" data-move-investment-account="${account.id}" data-direction="-1" aria-label="Move ${name} up" title="Move up"${index === 0 ? ' disabled' : ''}><svg viewBox="0 0 24 24" aria-hidden="true"><path d="m6 15 6-6 6 6"/></svg></button><button type="button" class="button ghost investment-order-button" data-move-investment-account="${account.id}" data-direction="1" aria-label="Move ${name} down" title="Move down"${index === total - 1 ? ' disabled' : ''}><svg viewBox="0 0 24 24" aria-hidden="true"><path d="m6 9 6 6 6-6"/></svg></button><button type="button" class="button ghost investment-archive-button" data-archive-investment-account="${account.id}">${account.archived ? 'Restore' : 'Archive'}</button></div></div>`;
  };
  const moveInvestmentAccount = (id, direction) => {
    const account = accountById(id);
    if (!account) return;
    const peers = data.settings.investmentAccounts.filter(item => item.archived === account.archived),
      peerIndex = peers.findIndex(item => item.id === id),
      target = peers[peerIndex + direction];
    if (!target) return;
    const from = data.settings.investmentAccounts.findIndex(item => item.id === id),
      to = data.settings.investmentAccounts.findIndex(item => item.id === target.id);
    [data.settings.investmentAccounts[from], data.settings.investmentAccounts[to]] = [
      data.settings.investmentAccounts[to],
      data.settings.investmentAccounts[from],
    ];
    save();
    syncInvestmentSettings();
    render();
  };
  const renderInvestments = () => {
    ...
    summary.innerHTML =
      ...
      pieCard(
        ...
        contributionValues,
        contributionColorIndexes,
      );
    accountGrid.innerHTML = accounts.length
      ? accounts
          .map((account, index) => {
            const yearContribution = accountYearContribution(account, year),
              ...
              secondaryStats =
                goal > 0
                  ? `<span class="investment-account-stat"><span>Added this year</span><strong>${money(yearContribution)}</strong></span><span class="investment-account-stat"><span>${is401k(account) ? 'Yearly contribution' : 'Yearly goal'}</span><strong>${money(goal)}</strong></span>`
                  : '';
    ...
    entriesHost.innerHTML = sorted.length
      ? sorted
          .map(entry => {
            ...
            return `<article class="history-entry investment-entry"><span class="history-date"><b>${date}</b><time>Your contribution</time></span><span class="history-kind"><b>${escapeHtml(account?.name || 'Archived account')}</b><span>${escapeHtml(account?.type || 'Account')}</span></span><b class="history-final">${money(entry.amount)}</b><div class="history-actions"><button type="button" class="history-icon-button" data-edit-investment="${entry.id}" aria-label="Edit investment">${editIcon}</button><button type="button" class="history-icon-button history-delete" data-delete-investment="${entry.id}" aria-label="Delete investment">${deleteIcon}</button></div>${entry.description ? `<p class="history-comment">${escapeHtml(entry.description)}</p>` : ''}</article>`;
          })
          .join('')
      : '<div class="chart-empty">Investments you add will appear here.</div>';
    const active = data.settings.investmentAccounts.filter(account => !account.archived),
      archived = data.settings.investmentAccounts.filter(account => account.archived),
      renderSettingsRows = accounts =>
        accounts.map((account, index) => settingsRow(account, index, accounts.length)).join('');
    settingsHost.innerHTML =
      renderSettingsRows(active) +
      (archived.length
        ? `<h4 class="investment-archived-heading">Archived</h4>${renderSettingsRows(archived)}`
        : '');
  };
  const syncHeaderAction = () => {
    if (!headerAction) return;
    ...
    if (activeTab === 'investments') {
      headerAction.textContent = '+ Add investment';
      headerAction.onclick = () => openInvestment();
    } else if (headerAction.textContent === '+ Add investment') {
      headerAction.textContent = '+ Add expense';
      headerAction.onclick = expenseHeaderAction;
    }
  };

  document.getElementById('investmentAccountOptions')?.addEventListener('click', event => {
    const button = event.target.closest('[data-investment-account]');
    if (button) selectInvestmentAccount(button.dataset.investmentAccount);
  });
  document.getElementById('closeInvestment').onclick = () => investmentDialog.close();
  document.getElementById('cancelInvestment').onclick = () => investmentDialog.close();
  investmentDialog.addEventListener('click', event => {
    if (event.target === investmentDialog) investmentDialog.close();
  });
  investmentForm.addEventListener('submit', event => {
    event.preventDefault();
    const existing = editingInvestmentId
        ? data.investments.find(item => item.id === editingInvestmentId)
        : null,
      entry = normalizeInvestmentRecord({
        id: existing?.id || crypto.randomUUID(),
    ...
    if (!entry) return;
    if (existing) data.investments[data.investments.findIndex(item => item.id === existing.id)] = entry;
    else data.investments.unshift(entry);
    save();
    syncInvestmentRecord(entry);
    investmentDialog.close();
    render();
  });
  entriesHost.addEventListener('click', event => {
    const edit = event.target.closest('[data-edit-investment]'),
      remove = event.target.closest('[data-delete-investment]');
    if (edit) openInvestment(edit.dataset.editInvestment);
    if (remove) {
      const entry = data.investments.find(item => item.id === remove.dataset.deleteInvestment);
      if (!entry || !confirm('Delete this investment contribution?')) return;
      data.investments = data.investments.filter(item => item.id !== entry.id);
      save();
      deleteCloudInvestment(entry.id);
      render();
    }
  });
  document.getElementById('updateInvestmentTotals').onclick = event => {
    event.preventDefault();
    event.stopPropagation();
    openTotals();
  };
  document.getElementById('closeInvestmentTotals').onclick = () => totalsDialog.close();
  document.getElementById('cancelInvestmentTotals').onclick = () => totalsDialog.close();
  ...
  totalsForm.addEventListener('submit', event => {
    ...
    const date = localToday(),
      ...
      records = [...totalsFields.querySelectorAll('[data-balance-account]')]
        .map(input => {
          const account = accountById(input.dataset.balanceAccount),
            raw = input.value.trim(),
            total = raw === '' ? currentAccountTotal(account) : safeNumber(raw, 0),
            existing = data.investmentBalances.find(
              item => item.accountId === account.id && item.date === date,
            );
          return normalizeInvestmentBalanceRecord({
            id: existing?.id || `${account.id}-${date}`,
            accountId: account.id,
            date,
            total,
    ...
    records.forEach(record => {
      const index = data.investmentBalances.findIndex(
        item => item.accountId === record.accountId && item.date === record.date,
      );
      if (index >= 0) data.investmentBalances[index] = record;
      else data.investmentBalances.push(record);
      syncInvestmentBalanceRecord(record);
    });
    save();
    totalsDialog.close();
    render();
  });
  document.getElementById('addInvestmentAccount').onclick = () => {
    accountForm.reset();
    newAccountType.value = 'IRA';
    accountDialog.showModal();
    requestAnimationFrame(() => newAccountName.focus());
  };
  document.getElementById('closeInvestmentAccount').onclick = () => accountDialog.close();
  document.getElementById('cancelInvestmentAccount').onclick = () => accountDialog.close();
  accountDialog.addEventListener('click', event => {
    if (event.target === accountDialog) accountDialog.close();
  });
  accountForm.addEventListener('submit', event => {
    event.preventDefault();
    const account = normalizeInvestmentAccount({
      id: `investment-account-${crypto.randomUUID()}`,
    ...
    if (!account) return;
    data.settings.investmentAccounts.push(account);
    save();
    syncInvestmentSettings();
    accountDialog.close();
    render();
  ...
  settingsHost.addEventListener('change', event => {
    const input = event.target.closest('[data-investment-setting]');
    if (!input) return;
    const account = accountById(input.dataset.investmentAccountId);
    if (!account) return;
    const field = input.dataset.investmentSetting;
    if (field === 'name') account.name = safeText(input.value || 'Account', 60).trim() || 'Account';
    else if (field === 'type' && investmentAccountTypes.includes(input.value)) {
      account.type = input.value;
      if (is401k(account)) account.annualGoal = 0;
      else account.yearlyContribution = 0;
      if (isSavings(account)) account.annualGoal = 0;
    } else account[field] = safeNumber(input.value, 0);
    save();
    syncInvestmentSettings();
    render();
  });
  settingsHost.addEventListener('click', event => {
    const move = event.target.closest('[data-move-investment-account]');
    if (move) {
      moveInvestmentAccount(move.dataset.moveInvestmentAccount, Number(move.dataset.direction));
      return;
    }
    const button = event.target.closest('[data-archive-investment-account]');
    if (!button) return;
    const account = accountById(button.dataset.archiveInvestmentAccount);
    if (!account) return;
    if (
      !account.archived &&
      !confirm(`Archive ${account.name}? Its contribution and balance history will be preserved.`)
    )
      return;
...
(() => {
  ...
  const spendingTabs = () =>
    `<div class="bill-tabs spending-tabs">${[
      ['all', 'All'],
      ['travel', 'Travel'],
      ['dining', 'Dining'],
    ]
      .map(
        ([value, label]) =>
          `<button class="${spendingMetric === value ? 'selected' : ''}" data-spending-series="${value}">${label}</button>`,
      )
      .join('')}</div>`;
  ...
  renderCharts = () => {
    previousRenderCharts();
    document.getElementById('categoryBarsChart')?.remove();
    const spendingCard = [...document.querySelectorAll('#chartArea .chart-grid .chart-card')].find(
      card => card.querySelector('h3')?.textContent.trim() === 'Spending',
    );
    if (spendingCard) {
      ...
      if (spendingMetric === 'travel') {
        const replacement = [...document.querySelectorAll('#chartArea .chart-grid .chart-card')].find(
          card => card.querySelector('h3')?.textContent.trim() === 'Spending',
        );
...
(() => {
  ...
  const renderRandomInsights = () => {
    ...
    const latest = completed.at(-1);
    if (!latest) {
      insightHost.classList.add('hidden');
      return;
    }
    const messages = [];
    ...
    if (previous) {
      ...
      if (num(latest.groceries) || num(previous.groceries))
        addMonthlyChange('Groceries', latest.groceries, previous.groceries);
      ...
      if (latestUtilities || previousUtilities)
        addMonthlyChange('Your combined utility bills', latestUtilities, previousUtilities);
      ...
      if (latestIncome > 0 && previousIncome > 0) {
        const latestRate = round((total(latest).saved / latestIncome) * 100),
          previousRate = round((total(previous).saved / previousIncome) * 100),
      ...
      const entryTotal = (monthId, predicate) =>
        round(
          data.entries
            .filter(entry => entry.month === monthId && entry.type === 'Spending' && predicate(entry))
            .reduce((sum, entry) => sum + num(entry.final), 0),
        );
      const latestDining = entryTotal(latest.id, entry => entry.category === 'Dining'),
        previousDining = entryTotal(previous.id, entry => entry.category === 'Dining');
      if (latestDining || previousDining) addMonthlyChange('Dining', latestDining, previousDining);
      const extraIncomeFor = monthId =>
        round(
          data.extraIncome
            .filter(entry => entry.month === monthId)
            .reduce((sum, entry) => sum + num(entry.amount), 0),
        );
      const latestExtra = extraIncomeFor(latest.id),
        previousExtra = extraIncomeFor(previous.id);
    ...
    if (lastYearMonth && num(lastYearMonth.groceries) > 0) {
      const change = round(
        ((num(latest.groceries) - num(lastYearMonth.groceries)) / num(lastYearMonth.groceries)) * 100,
      );
      if (change)
        messages.push(
    ...
    const year = Number(latest.id.slice(0, 4)),
      cutoff = latest.id.slice(5);
    const thisYear = completed.filter(
      month => month.id.startsWith(`${year}-`) && month.id.slice(5) <= cutoff,
    );
    ...
    data.investmentBalances.forEach(entry => {
      ...
      if (!balanceSnapshots.has(entry.date))
        balanceSnapshots.set(entry.date, { total: 0, accounts: new Set() });
      const snapshot = balanceSnapshots.get(entry.date);
      snapshot.total += num(entry.total);
      snapshot.accounts.add(entry.accountId);
    });
    const completeSnapshots = [...balanceSnapshots.entries()]
      .filter(([, snapshot]) => activeAccountIds.size && snapshot.accounts.size === activeAccountIds.size)
      .sort(([a], [b]) => a.localeCompare(b));
    if (completeSnapshots.length >= 2) {
      const [previousDate, previousSnapshot] = completeSnapshots.at(-2),
        [latestDate, latestSnapshot] = completeSnapshots.at(-1),
        difference = round(latestSnapshot.total - previousSnapshot.total);
      if (Math.abs(difference) >= 1) {
        const since = new Date(`${previousDate}T12:00:00`).toLocaleDateString('en-US', {
          month: 'short',
          day: 'numeric',
        });
        messages.push(
    ...
    insightHost.innerHTML = selectedInsights
      .map(message => `<p class="overview-insight">${message}</p>`)
      .join('');
...
(() => {
  const highlightCurrentMonth = () => {
    document
      .querySelectorAll('.monthly-table tr.current-month-row')
      .forEach(row => row.classList.remove('current-month-row'));
    document
      .querySelector(`.monthly-table [data-month="${currentMonthKey()}"]`)
      ?.closest('tr')
...
(() => {
  ...
  const cardIcon =
    '<svg viewBox="0 0 24 24" fill="none" aria-hidden="true"><rect x="3" y="5" width="18" height="14" rx="2" stroke="currentColor" stroke-width="1.7"/><path d="M3 10h18M7 15h4" stroke="currentColor" stroke-width="1.7" stroke-linecap="round"/></svg>';
  let editingAccountId = '';
  let draftSvg = '';

  const svgUrl = svg => (svg ? `data:image/svg+xml;charset=utf-8,${encodeURIComponent(svg)}` : '');
  const artwork = (account, preview = false) =>
    account?.imageSvg
      ? `<img src="${svgUrl(account.imageSvg)}" alt="${preview ? '' : 'Artwork for ' + escapeHtml(account.name)}">`
      : `<span class="account-placeholder">${cardIcon}</span>`;
  const selectKind = kind => {
    kindInput.value = kind;
    document
      .querySelectorAll('[data-account-kind]')
      .forEach(button => button.classList.toggle('selected', button.dataset.accountKind === kind));
  };
  const renderPreview = () => {
    imagePreview.innerHTML = artwork(
      draftSvg ? { imageSvg: draftSvg, name: nameInput.value || 'Account' } : null,
      true,
    );
    document.getElementById('removeAccountImage').disabled = !draftSvg;
  };
  ...
  document.querySelector('.account-kind-options')?.addEventListener('click', event => {
    const button = event.target.closest('[data-account-kind]');
    if (button) selectKind(button.dataset.accountKind);
  });
  imageInput.addEventListener('change', async () => {
    ...
    if (!file.name.toLowerCase().endsWith('.svg') && file.type !== 'image/svg+xml') {
      imageStatus.textContent = 'Choose an SVG file.';
      return;
    }
    if (file.size > 100000) {
      imageStatus.textContent = 'That SVG is over the 100 KB limit.';
      return;
    }
    const cleaned = sanitizeAccountSvgMarkup(await file.text());
    if (!cleaned) {
      imageStatus.textContent = 'That SVG could not be safely imported.';
      return;
    }
    draftSvg = cleaned;
    imageStatus.textContent = 'SVG cleaned and ready to save.';
    renderPreview();
  });
  document.getElementById('removeAccountImage').onclick = () => {
    draftSvg = '';
    imageStatus.textContent = 'Image removed. Save to apply.';
    renderPreview();
  };
  ...
  form.addEventListener('submit', event => {
    ...
    if (!account) return;
    if (existing) data.accounts[data.accounts.findIndex(item => item.id === existing.id)] = account;
    else data.accounts.push(account);
    save();
    syncAccountRecord(account);
    dialog.close();
    render();
  });
  content?.addEventListener('click', event => {
    const edit = event.target.closest('[data-edit-account]'),
      remove = event.target.closest('[data-delete-account]');
    if (edit) {
      openAccount(edit.dataset.editAccount);
      return;
    }
    if (remove) {
      const account = data.accounts.find(item => item.id === remove.dataset.deleteAccount);
      if (!account || !confirm(`Delete ${account.name}?`)) return;
      data.accounts = data.accounts.filter(item => item.id !== account.id);
      save();
      deleteCloudAccount(account.id);
      render();
    }
  });
  document.addEventListener('click', event => {
  ...
  document.addEventListener(
    'change',
    event => {
      if (event.target?.id !== 'restoreBackupFile') return;
      event.stopImmediatePropagation();
      restoreBackup(event).then(() =>
        firebaseClient && firebaseUser
          ? Promise.all([
              ...data.accounts.map(syncAccountRecord),
              ...data.investmentBalances.map(syncInvestmentBalanceRecord),
            ])
          : null,
      );
    },
